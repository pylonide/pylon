...
var apf = {
    ...
    */
    ...
    include : function(sourceFile, doBase, type){
        apf.console.info(document.title = "including js file: " + sourceFile);
...
/*(function(global){
    ...
    function canonicalScriptURI(src,base_path) {
        ...
        return ((/^\w+\:\/\//.test(ret) ? "" : (ret.charAt(0) === "/" ? page_domain : page_path)) + ret);
    }
    function sameDomain(src) { return (canonicalScriptURI(src).indexOf(page_domain) === 0); }
		...
		script:function(){ // will load one or more scripts
			for (var i = 0, l = arguments.length; i < l; i++) {
			    if (typeof arguments[i] == "string") {
			        apf.include(apf.getAbsolutePath(this.$settings.BasePath || "", arguments[i]));
			    }
			    else {
			        for (var a = arguments[i], j = 0, jl = a.length; j < jl; j++){ 
			            apf.include(apf.getAbsolutePath(this.$settings.BasePath || "", a[j]));
			        }
			    }
			}
			return this;
