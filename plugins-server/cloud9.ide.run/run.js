...
module.exports = function (vfs, ide, callback) {
    ...
    var RunPlugin = function (ide, workspace) {
        ...
        this.command = function(user, message, client) {
            ...
            else if (message.type.search(/^internal-\w+/) !== -1) {
                var match = message.type.match(/^internal-(\w+)/);
                if (match && match[1])
                    this["internal_" + match[1]](message);
            }
        };

        this.createChild = function(cmd, runOptions, callback) {
            var self = this;
            vfs.spawn(cmd, runOptions, function(err, meta) {
...
*/
    ...
    ide.register("run", RunPlugin, callback);
};