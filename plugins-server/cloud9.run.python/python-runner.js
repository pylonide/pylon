...
/**
 * Run python scripts with restricted user rights
 */
...
var exports = module.exports = function (url, pm, sandbox, usePortFlag, callback) {
    ...
    function init(projectDir, unixId, url) {
        pm.addRunner("python", exports.factory(sandbox, projectDir, unixId, url, usePortFlag));
...
exports.factory = function(sandbox, root, uid, url, usePortFlag) {
    return function(args, eventEmitter, eventName, callback) {
        ...
        options.pythonVersion = args.pythonVersion;
...
var Runner = exports.Runner = function(options, callback) {
    ...
    self.pythonVersion = options.pythonVersion || "auto";

    self.file = options.file || "";

    options.env = options.env || {};
    options.env.python_PATH = options.root && (options.root + "/../npm_global/lib/python_modules");

    self.scriptArgs = options.args || [];
    self.pythonArgs = [];

    if (options.uid) {
        self.pythonArgs.push("--setuid=" + options.uid);    
    }
    ...
    options.sandbox.getPort(function (err, port) {
        ...
        if (options.usePortFlag) {
            self.pythonArgs.push("--ports=" + port);
        }
    ...
    function startProcess (url, port) {
        ...
        if (options.usePortFlag) {
            self.pythonArgs.push("--ports=" + self.port);
        }
