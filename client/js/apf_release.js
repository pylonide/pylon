...
for(i=nodes.length-1;i>=0;i--){parentNode.removeChild(nodes[i]);}}beforeNode=options&&options.beforeNode?options.beforeNode:apf.getNode(parentNode,[0]);
...
}});}else{apf.setModel(value,this);}};};apf.DataBinding.prototype=new apf[apf.Presentation?"Presentation":"AmlElement"]();
...
}while(xmlNode&&xmlNode.nodeType!=9);var foundNode=xmlNode;if(xmlNode&&xmlNode.nodeType==9){xmlNode=startNode;
}if(action=="replacenode"){if(UndoObj?UndoObj.args[1]==this.xmlRoot:!this.xmlRoot.parentNode){return this.load(UndoObj?UndoObj.xmlNode:listenNode,{force:true});
...
}this.remove();break;case 36:var node=this.getFirstTraverseNode();this.select(node,false,shiftKey);
this.$container.scrollTop=0;break;case 35:var node=this.getLastTraverseNode();this.select(node,false,shiftKey);
this.$container.scrollTop=this.$container.scrollHeight;break;case 107:if(this.more){this.startMore();
}break;case 37:if(!selXml&&!this.$tempsel){return;}node=this.$tempsel?apf.xmldb.getNode(this.$tempsel):selXml;
margin=apf.getBox(apf.getStyle(selHtml,"margin"));items=selHtml.offsetWidth?Math.floor((oExt.offsetWidth-(hasScroll?15:0))/(selHtml.offsetWidth+margin[1]+margin[3])):1;
...
}});this.attach=function(viewport){if(viewport.nodeFunc){viewport=new apf.ViewPortAml(viewport);
}else{if(viewport.style){viewport=new apf.ViewPortHtml(viewport);}}this.$attach(viewport);
};this.$attach=function(viewport){if(!viewport){return apf.console.warn("Scrollbar could not connect to viewport");
