...
}apf.setProperty("hotkey",keys.join("-"));}});}).call(apf.hotkeys);apf.nameserver={lookup:{},add:function(type,item){if(!this.lookup[type]){this.lookup[type]=[];
...
}}else{if(action=="add"){var parentHTMLNode;pNode=this.getTraverseParent(xmlNode)||this.xmlRoot;
...
this.margin="10 10 10 10";this.startPadding=0;this.lastPos=null;this.showing=0;this.sign=1;this.$supportedProperties.push("margin","position","timeout","columnsize","arrange","start-padding");
this.$propHandlers.position=function(value){this.lastPos=null;};this.$propHandlers.margin=function(value){this.margin=value;
};this.$propHandlers["start-padding"]=function(value){this.startPadding=parseInt(value);};this.$propHandlers.timeout=function(value){this.timeout=parseInt(value)*1000;
};var _self=this;function getPageScroll(){return[document.documentElement.scrollTop||document.body.scrollTop,document.documentElement.scrollLeft||document.body.scrollLeft];
}function getStartPosition(x,wh,ww,nh,nw,margin,startPadding){console.log(_self.arrange );var scrolled=getPageScroll();return[(x[0]=="top"?margin[0]:(x[0]=="bottom"?wh-nh-margin[2]:wh/2-nh/2))+scrolled[0] + (_self.arrange == "vertical" ? startPadding : 0),(x[1]=="left"?margin[3]:(x[1]=="right"?ww-nw-margin[1]:ww/2-nw/2))+scrolled[1]+ (_self.arrange == "horizontal" ? startPadding : 0)];
...
}if(!this.lastPos){this.lastPos=getStartPosition(x,wh,ww,nh,nw,margin,this.startPadding);this.sign=1;
...
}}if(this.lastPos){if((this.lastPos[0]>wh+scrolled[0]-nh||this.lastPos[0]<scrolled[1])&&this.arrange=="horizontal"){this.lastPos=getStartPosition(x,wh,ww,nh,nw,margin,this.startPadding);
this.sign=1;}if((this.lastPos[1]>ww+scrolled[1]-nw||this.lastPos[1]<scrolled[1])&&this.arrange=="vertical"){this.lastPos=getStartPosition(x,wh,ww,nh,nw,margin,this.startPadding);
