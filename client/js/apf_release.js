...
this.cache=null;this.inited=false;this.setScrollbar=function(scrollbar,onscroll){this.scrollbar=scrollbar;
...
menu.display(null,pos[1]+this.$ext.offsetHeight,true,this,null,null,this.$ext.offsetWidth-2);
...
if(this.$rendered!==false){this.show();afterRender.call(this);}else{this.addEventListener("afterrender",afterRender);
this.show();}};this.getValue=function(group){return this.getSelected(group).value||"";
};this.getSelected=function(group){var nodes=this.childNodes;var i,l=nodes.length;
for(i=0;i<l;i++){if(nodes[i].group!=group){continue;}if(nodes[i].selected){return nodes[i];
}}return false;};this.select=function(group,value){var nodes=this.childNodes;var i,l=nodes.length;
for(i=0;i<l;i++){if(nodes[i].group!=group){continue;}if(nodes[i].value==value||!nodes[i].value&&nodes[i].caption==value){nodes[i].setProperty("selected",true,false,true);
...
}else{if(this.src){this.$loadFrom(this.src,{callback:callback});}}};this.$loadFrom=function(instruction,options){if(instruction.indexOf("rdb://")===0){this.src=instruction;
...
this.offlineQueue=[];};apf.remote.SESSION_INITED=1;apf.remote.SESSION_STARTED=2;
apf.remote.SESSION_TERMINATED=4;(function(){this.logprefix="";if(!apf.isO3){this.log=function(msg){apf.console.log(msg);
...
}var uri=e.uri,oSession=_self.$sessions[e.uri];if(!e.document){return _self.dispatchEvent("joinrequest",e);
}if(!oSession){oSession=_self.createSession(uri,null,null,e.document,e.basetime);
...
});};this.$update=function(e){var sData=e.message.args?[e.message]:e.message;var oData=typeof sData=="string"?apf.unserialize(sData):sData;
var oSession=this.$sessions[e.uri];var i=0;var l=oData.length;for(;i<l;i++){this.$receiveChange(oData[i],oSession,e.annotator);
}};this.clear=function(){this.$sessions={};};this.createSession=function(uri,model,xpath,doc,iTime){this.log&&this.log(this.logprefix+"Creating session for "+uri);
...
};this.$startSession=function(uri,basetime){var oSession=this.$sessions[uri];if(!oSession){return false;
}oSession.state=apf.remote.SESSION_STARTED;if(basetime&&!oSession.basetime){oSession.basetime=basetime;
}};this.$queueMessage=function(args,model,qHost){if(!qHost.rdbQueue){qHost.rdbQueue={};
}var uri=model.src;var oSession=this.$sessions[uri];if(!qHost.rdbQueue[uri]){qHost.rdbQueue[uri]=[];
qHost.rdbModel=model;}for(var node,i=0,l=args.length;i<l;++i){if((node=args[i])&&node.nodeType){if(args[0]=="setAttribute"&&args[2]=="level"&&args[1]==args[1].ownerDocument.documentElement){return false;
...
}}}}}qHost.rdbQueue[uri].push({uri:uri,args:args,currdelta:(new Date()).toISOString()-oSession.basetime});
...
}if(this.transport){this.transport.sendUpdate(uri,JSON.stringify(list));}this.dispatchEvent("rdbsend",{uri:uri,message:list});
...
if(!oSession&&oMessage.uri){oSession=this.$sessions[oMessage.uri];}if(!oSession){return;
}var model=oSession.model;if(!model){return;}if(!model.$at){model.$at=apf.window.$at;
}var oError,xmlNode,disableRDB=apf.xmldb.disableRDB;apf.xmldb.disableRDB=2;var time=oSession.basetime+parseInt(oMessage.currdelta,10);
var aUndos=[];var i=0;var l=aUndos.length;if(l){for(;i<l;++i){aUndos[i].$dontapply=true;
}model.$at.undo(l);}var q=oMessage.args.slice();var xpath=q[1];xmlNode=q[1]=this.xpathToXml(xpath,model.data);
...
this.xpathToXml=apf.xpathToXml;this.addEventListener("DOMNodeInsertedIntoDocument",function(e){});
this.addEventListener("DOMNodeRemovedFromDocument",function(e){for(var i=0,l=this.$sessions.length;
...
}}var options,_self=this;this.processRsbQueue=function(){if(this.rdbModel){this.rdbModel.rdb.$processQueue(this);
...
}}apf.xmldb.delayUpdate=dUpdate;}};apf.cgi=function(){this.supportMulticall=false;
