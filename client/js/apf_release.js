...
this.supportOpacity=!apf.isIE;this.supportPng24=!apf.isIE6&&!apf.isIE5;this.cantParseXmlDefinition=apf.isIE50;
...
}}var styleSheet=doc.styleSheets[index];if(!styleSheet){console.log("create");if(doc.createStyleSheet){styleSheet=doc.createStyleSheet();
...
}this.transition=function(pageIn,animIn,pageOut,animOut){var _self=this;if(!this.$transInfo){this.$int.style.overflow="hidden";
...
}else{if(ext.style.right){ext.style.left=myPos[0]+"px";}if(ext.style.bottom){ext.style.top=myPos[1]+"px";
...
}}return res;}else{arg1=nodelist;}}switch(type){case"position":return apf.getChildNumber(contextNode)+1;
...
}search.push(words.join(" or "));}var filter="("+search.join(") and (")+")";each=each.split("|");
var newEach=[];for(i=0,l=each.length;i<l;i++){newEach.push("("+each[i]+")["+filter+"]");
...
};this.$update=function(){if(this.animating||!this.$visible){return;}var oHtml=this.$getHtmlHost();
if(!oHtml||!oHtml.offsetHeight){return;}this.$updating=true;var vp=this.$getViewPort(oHtml);
var sz=this.$getScrollHeight(oHtml);if(vp>=sz){if(this.overflow=="scroll"){this.$caret.style.display="none";
...
}else{if(iStatus==403||iStatus==405||iStatus==409||iStatus==415||iStatus==507){var oError=WebDAVError.call(this,"Unable to create directory '"+sPath+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
...
var iStatus=parseInt(extra.status);if(iStatus==403||iStatus==409||iStatus==412||iStatus==423||iStatus==424||iStatus==502||iStatus==507){var oError=WebDAVError.call(this,"Unable to copy file '"+sFrom+"' to '"+sTo+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false){throw oError;
}}else{this.getProperties(sTo,0,callback);}},sFrom,null,oHeaders);};this.rename=this.move=function(sFrom,sTo,bOverwrite,bLock,callback){if(!sTo||sFrom==sTo){return(callback&&callback("",apf.SUCCESS,{}));
...
var iStatus=parseInt(extra.status);if(iStatus==403||iStatus==409||iStatus==412||iStatus==423||iStatus==424||iStatus==502||iStatus==500){var oError=WebDAVError.call(this,"Unable to move file '"+sFrom+"' to '"+sTo+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false){throw oError;
...
var iStatus=parseInt(extra.status);if(iStatus==423||iStatus==424){var oError=WebDAVError.call(this,"Unable to remove file '"+sPath+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false){throw oError;
...
if(iStatus==409||iStatus==423||iStatus==412){unregisterLock.call(this,extra.url.replace(this.$server,""));
