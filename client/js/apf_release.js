...
this.addEventListener("focus",function(){if(this.$host.focus&&this.$host.$isWindowContainer!==true){this.$host.focus();
}});this.$propHandlers.showonscroll=function(value){clearTimeout(this.$hideOnScrollTimer);
...
_self.setScroll(null,null,null,true);e.preventDefault();}};var _self=this,scrolling;
if(!this.$host.empty){amlNode.addEventListener("resize",function(){_self.$update();
...
}});}}var oHtml=_self.$getHtmlHost();apf.addListener(oHtml,"scroll",function(){if(_self.animating||!_self.$visible){return;
...
_self.setScroll(null,null,null,true);}return false;}scrolling=false;});if("HTML|BODY".indexOf(oHtml.tagName)>-1){var lastHeight=oHtml.scrollHeight;
...
}var oHtml=this.$getHtmlHost();if(!oHtml||!oHtml.offsetHeight){return;}this.$updating=true;
var vp=this.$getViewPort(oHtml);var sz=this.$getScrollHeight(oHtml);if(vp>=sz){if(this.overflow=="scroll"){this.$caret.style.display="none";
...
}this.$updating=false;};this.setScroll=function(timed,noEvent,noUpdateParent,byUser){if(this.$curValue>1){this.$curValue=1;
...
}if(this.showonscroll&&byUser){var _self=this;this.scrolling=true;clearTimeout(this.$hideOnScrollTimer);
if(_self.$hideOnScrollControl){_self.$hideOnScrollControl.stop();}apf.setOpacity(this.$ext,1);
!this.visible?this.show():this.$ext.style.display="block";this.$update();this.$hideOnScrollTimer=this.animHideScrollbar(500,function(){_self.scrolling=false;
});}this.pos=this.$curValue;};this.animShowScrollbar=function(timeout,cb){var _self=this;
...
}if(_self.$showOnScrollControl){_self.$showOnScrollControl.stop();}apf.tween.single(_self.$ext,{control:_self.$hideOnScrollControl={},type:"fade",from:1,to:0,onfinish:function(){_self.$ext.style.display="none";
...
}this.$curValue-=this.$bigStepValue;this.setScroll(null,null,null,true);if(this.$slideFast){this.$slideFast.style[this.$size]=Math.max(1,this.$caret[this.$offsetPos]-this.$btnUp[this.$offsetSize])+"px";
...
}this.$curValue+=this.$bigStepValue;this.setScroll(null,null,null,true);if(this.$slideFast){this.$slideFast.style[this.$pos]=(this.$caret[this.$offsetPos]+this.$caret[this.$offsetSize])+"px";
...
_self.setScroll(null,null,null,true);apf.stopPropagation(e);_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.$curValue-=_self.$stepValue;
_self.setScroll(null,null,null,true);},20);},300);};this.$btnUp.onmouseout=this.$btnUp.onmouseup=function(){if(_self.disabled){return;
...
_self.$curValue+=_self.$stepValue;_self.setScroll(null,null,null,true);apf.stopPropagation(e);
_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.$curValue+=_self.$stepValue;
_self.setScroll(null,null,null,true);},20);},300);};this.$btnDown.onmouseout=this.$btnDown.onmouseup=function(){if(_self.disabled){return;
...
_self.dispatchEvent("mouseup",{});_self.dragging=false;document.onmouseup=document.onmousemove=null;
...
_self.setScroll(true,null,null,true);if(_self.$slideFast){_self.$slideFast.style.display="block";
...
if(!_self.realtime){_self.setScroll(null,null,null,true);}if(_self.$slideFast){_self.$slideFast.style.display="none";
}};this.$ext.onmouseover=function(e){_self.dispatchEvent("mouseover",{htmlEvent:e||event});
};this.$ext.onmouseout=function(e){_self.dispatchEvent("mouseout",{htmlEvent:e||event});
...
var show=oHtml.offsetWidth-(e.clientX-pos[0])<25;if(show&&sb.$ext.style.display=="none"||!show&&sb.$ext.style.display=="block"){if(show){showScrollbar();
}else{hideScrollbar();}}else{if(!show){sb.showonscroll=true;}}}});function showScrollbar(){sb.setProperty("showonscroll",false);
sb.$ext.style.display="none";timer=sb.animShowScrollbar(200);}function hideScrollbar(timeout){if(sb.scrolling){return;
...
});}}apf.addListener(oHtml,"mouseout",function(e){if(!hasOnScroll()){return;}if(apf.findHost(e.relatedTarget)==sb){apf.addListener(sb.$ext,"mouseout",function(e){hideScrollbar();
apf.removeListener(sb.$ext,"mouseout",arguments.callee);});return;}clearTimeout(timer);
hideScrollbar();});}};apf.skin=function(struct,tagName){this.$init(tagName||"skin",apf.NODE_HIDDEN,struct);
