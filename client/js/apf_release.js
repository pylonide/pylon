...
this.supportCanvas=!!document.createElement("canvas").getContext;this.supportCanvasText=!!(this.supportCanvas&&typeof document.createElement("canvas").getContext("2d").fillText=="function");
...
}if(apf.isWebkit){apf.runWebkit();}if(this.isGecko||!this.isIE&&!apf.isWebkit&&!this.isOpera){apf.runGecko();
}this.oHttp=new this.http();this.Init.addConditional(this.parseAppMarkup,apf,["body"]);
try{if(apf.isIE){document.execCommand("BackgroundImageCache",false,true);}}catch(e){}this.started=true;
if(this.load_done){this.execDeferred();}this.root=true;for(var i=0;i<apf.$required.length;
i++){apf.include(apf.$required[i]);}apf.require=apf.include;},nsqueue:{},findPrefix:function(xmlNode,xmlns){var docEl;
if(xmlNode.nodeType==9){if(!xmlNode.documentElement){return false;}if(xmlNode.documentElement.namespaceURI==xmlns){return xmlNode.prefix||xmlNode.scopeName;
...
},getCtrlKey:function(event){return apf.isMac?event.metaKey:event.ctrlKey;},include:function(sourceFile,doBase,type,text,callback){var sSrc=doBase?apf.getAbsolutePath(apf.basePath||"",sourceFile):sourceFile;
...
apf.window.init(xmlStr);},execDeferred:function(){var len=apf.load_events.length;
...
}else{el.fireEvent("on"+type,e);}},addListener:function(el,type,fn,capture){if(el.addEventListener){el.addEventListener(type,fn,capture||false);
}else{if(el.attachEvent){el.attachEvent("on"+type,fn);}}return this;},removeListener:function(el,type,fn,capture){if(el.removeEventListener){el.removeEventListener(type,fn,capture||false);
...
if(this.ownerDocument&&this.ownerDocument.$domParser){this.ownerDocument.$domParser.$continueParsing(this);
}apf.queue.empty();}return this;};this.implement=apf.implement;this.$handlePropSet=function(prop,value){this[prop]=value;
...
}isBeingCalled=true;_self.setProperty(myProp,bObject[bProp],false,false,10);isBeingCalled=false;
...
}var fParsed=apf.lm.compile(pValue,options);if(prop==MODEL){(this.$modelParsed=fParsed).instruction=pValue;
...
node=self[o[0]]||apf.nameserver.get("all",o[0]);}else{o.push(bProp);}}else{bProp=o[1];
node=self[o[0]]||apf.nameserver.get("all",o[0])||o[0]=="this"&&this;}if(!node){if(arguments[2]){apf.console.warn("[287] Could not create property binding:  '"+o[0]+"' does not exist. \n"+pValue.replace(/</g,"&lt;").substr(0,400));
...
}}delete this.$funcHandlers[prop];}};this.getAvailableProperties=function(){return this.$supportedProperties.slice();
...
}}}if(setAttr&&!this.$funcHandlers[prop]){this.setAttribute(prop,value,true);}if(this.$handlePropSet(prop,value,forceOnMe)===false){return;
...
if(!apf.AmlEvent){return;}apf.$eventDepth++;this.$eventDepth++;e=options&&options.name?options:e;
...
var len=6-hex.length;if(len>0){var ch="0";var o=[];var i=0;if(asBrowser){ch=hex.charAt(hex.length-1);
o.push(hex);}for(;i<len;i++){o.push(ch);}if(!asBrowser){o.push(hex);}hex=o.join("");
}return hex;},hexToRGB:function(hex){hex=parseInt(((hex.indexOf("#")>-1)?hex.substring(1):hex),16);
...
},HSBToHex:function(hsb){return this.RGBToHex(this.HSBToRGB(hsb));}};apf.asyncForEach=function(list,async,callback){var i=0;
var len=list.length;if(!len){return callback(null,[]);}async(list[i],function handler(err){if(err){return callback(err);
}i++;if(i<len){async(list[i],handler,i);}else{callback(null);}},i);};apf.asyncWhile=function(condition,async,callback){var i=0;
...
return function(event){if(!event){event=window.event;}return __method.apply(o,[event].concat(args).concat(Array.prototype.slice.call(arguments)));
...
i<l;i++){if(fn.call(this,this[i],i,this)===false){break;}}return this;};Array.prototype.remove=function(obj){for(var i=this.length-1;
...
var macUnicode={meta:"\u2318",command:"\u2318",alt:"\u2325",option:"\u2325",shift:"\u21E7",esc:"\u238B",ctrl:"\u2303",backspace:"\u232B",del:"\u2326",enter:"\u21A9"};
var macUnicodeHtml={meta:"&#8984;",command:"&#8984;",alt:"&#8997;",option:"&#8997;",shift:"&#8679;",esc:"&#9099;",ctrl:"&#2303;",backspace:"&#232B;",del:"&#2326;",enter:"&#21A9;"};
this.$keys={};var _self=this,trace=0;function register(hotkey,handler,remove){var key,hashId=0,keys=hotkey.splitSafe("\\-",null,true),i=0,l=keys.length;
...
if(apf.canDisableKeyCodes){try{e.keyCode=0;}catch(e){}}return false;}});}).call(apf.hotkeys);
apf.nameserver={lookup:{},add:function(type,item){if(!this.lookup[type]){this.lookup[type]=[];
...
}arr.push(name);}return arr;}};apf.plane={$set:[],$lookup:{},$find:function(id){if(this.$lookup[id]){return this.$lookup[id];
...
var item=this.$find(options&&options.protect||"default");item.show(o,reAppend,copyCursor,useRealSize,options);
},hide:function(protect,noAnim){var item=this.$lookup[protect||"default"];if(item){item.hide(noAnim);
delete this.$lookup[protect||"default"];this.$set.push(item);}},$factory:function(){var _self=this,spacerPath="url("+(apf.skins.skins["default"]?apf.skins.skins["default"].mediaPath+"spacer.gif":"images/spacer.gif")+")";
function getCover(){var obj=document.createElement("DIV");if(!_self.options||!_self.options.customCover){return obj;
}obj.innerHTML=apf.getXmlString(_self.options.customCover);return obj.firstChild;
}function createCover(){var cover=document.body.appendChild(getCover());if(!_self.options.customCover){cover.style.background=spacerPath;
}cover.style.position="fixed";cover.style.left=0;cover.style.top=0;cover.host=false;
return cover;}var plane=createCover();return{host:this,plane:plane,lastCursor:null,lastCoverType:"default",show:function(o,reAppend,copyCursor,useRealSize,options){var coverType=options&&options.customCover?"custom":"default",plane;
if(coverType=="custom"||this.lastCoverType!=coverType){this.plane=createCover();
}plane=this.plane;if(!options||!options.customCover){this.plane.style.background=options&&options.color||spacerPath;
}this.animate=options&&options.animate;this.protect=options&&options.protect;if(this.protect){apf.setProperty("planes",(apf.planes||0)+1);
...
this.plane.style.width="100%";this.plane.style.height="100%";this.lastCoverType=options&&options.customCover?"custom":"default";
return plane;},hide:function(noAnim){if(this.protect){apf.setProperty("planes",apf.planes-1);
...
content.onmousedown=function(e){if(!e){e=event;}};return content.ownerDocument;
},removeContent:function(cacheId){this.cache[cacheId]=null;delete this.cache[cacheId];
},init:function(){this.popup={};apf.addEventListener("hotkey",function(e){if(e.keyCode=="27"||e.altKey){apf.popup.forceHide();
}});},show:function(cacheId,options){if(!this.popup){this.init();}options=apf.extend({x:0,y:0,animate:false,ref:null,width:null,height:null,callback:null,draggable:false,resizable:false,allowTogether:false,autoCorrect:true,noleft:false,setZindex:true},options);
...
}var o=this.cache[cacheId];o.options=options;var dp,popup=o.content,moveUp=false,moveLeft=false,fixed=false;
if(options.setZindex){apf.window.zManager.set("popup",o.content);}if((dp=o.content.style.display)&&dp.indexOf("none")>-1){o.content.style.display="";
...
}popup.style.position="absolute";var parentMenu=this.cache[options.allowTogether];
var pOverflow=apf.getOverflowParent(o.content);var edgeY=(pOverflow==document.documentElement?(apf.isIE?pOverflow.offsetHeight:(window.innerHeight+window.pageYOffset))+pOverflow.scrollTop:pOverflow.offsetHeight+pOverflow.scrollTop);
moveUp=options.autoCorrect&&(y+(options.height||o.height||o.content.offsetHeight))>edgeY;
if(moveUp){var value;if(refNode){value=(pos[1]-(options.height||o.height||o.content.offsetHeight))+3;
...
}popup.style.display="block";if(options.callback){options.callback(popup);}}$setTimeout(function(){apf.popup.last=cacheId;
});if(options.draggable){options.id=cacheId;this.makeDraggable(options);}},hide:function(){if(this.isDragging){return;
}var o=this.cache[this.last];if(o){if(o.content){o.content.style.display="none";
}if(o.options&&o.options.onclose){o.options.onclose(apf.extend(o.options,{htmlNode:o.content}));
...
oHtml.onmousedown=oHtml.firstChild.onmousedown=function(e){if(!e){e=event;}(e||event).cancelBubble=true;
};apf.implement.call(o,apf.Interactive);o.$propHandlers.draggable.call(o,true);
...
i++){if(rules.item(i).selectorText&&rules.item(i).selectorText.toLowerCase()==name){rules.item(i).style[type]=value;
return true;}}}catch(e){}}}else{var rules=(win||self).document.styleSheets[stylesheet||0][apf.styleSheetRules];
for(var i=0;i<rules.length;i++){if(rules.item(i).selectorText&&rules.item(i).selectorText.toLowerCase()==name){rules.item(i).style[type]=value;
return true;}}}return false;};apf.getStyleRule=function(name,type,stylesheet,win){name=name.toLowerCase();
if(!stylesheet){var sheets=(win||self).document.styleSheets;for(var j=sheets.length-1;
j>=0;j--){try{var rules=sheets[j][apf.styleSheetRules];for(var i=0;i<rules.length;
i++){if(rules.item(i).selectorText&&rules.item(i).selectorText.toLowerCase()==name){return rules.item(i).style[type];
}}}catch(e){}}}else{var rules=(win||self).document.styleSheets[stylesheet||0][apf.styleSheetRules];
for(var i=0;i<rules.length;i++){if(rules.item(i).selectorText&&rules.item(i).selectorText.toLowerCase()==name){return rules.item(i).style[type];
...
var doc=(win||window).document;for(var index=document.styleSheets.length-1;index>=0;
index--){if(!doc.styleSheets[index].href||doc.styleSheets[index].href.match(re)){break;
}}var styleSheet=doc.styleSheets[index];function newStyleSheet(){if(doc.createStyleSheet){return doc.createStyleSheet();
}else{var elem=doc.createElement("style");elem.type="text/css";doc.getElementsByTagName("head")[0].appendChild(elem);
return elem.sheet;}}if(!styleSheet){styleSheet=newStyleSheet();}for(var i=0;i<def.length;
i++){if(!def[i][1]){continue;}if(apf.isIE){styleSheet.addRule(def[i][0],def[i][1]);
}else{var rule=def[i][0]+" {"+def[i][1]+"}";try{styleSheet.insertRule(rule,0);}catch(e){styleSheet=newStyleSheet();
styleSheet.insertRule(rule,0);}}}};apf.isInRect=function(oHtml,x,y){var pos=this.getAbsolutePosition(oHtml);
...
};apf.htmlentities=function(str){return str.escapeHTML();};apf.xmlentities=apf.escapeXML;
apf.html_entity_decode=function(str){return(str||"").replace(/\&\#38;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&").replace(/&nbsp;/g," ");
...
}var p=node.parentNode,j=0;if(!p){return -1;}if(!fromList){fromList=p.childNodes;
}if(fromList.indexOf){var idx=fromList.indexOf(node);return idx==-1?fromList.length:idx;
}for(var i=0,l=fromList.length;i<l;i++){if(fromList[i]==node){return j;}j++;}return -1;
...
}var oldValue=xmlNode.nodeValue;xmlNode.nodeValue=nodeValue==undefined||nodeValue==null||nodeValue==NaN?"":String(nodeValue);
...
};}).call(apf.xmlset.prototype);apf.zmanager=function(){var count={"default":{level:10},plane:{level:100000},popup:{level:200000},notifier:{level:300000},drag:{level:400000},print:{level:1000000}};
this.set=function(type,main,companion){main.style.zIndex=count[type].level++;if(companion){companion.$storedZ=companion.style.zIndex;
...
default:return m;}}):this._toString();};}());apf.layout={compile:function(oHtml){var l=this.layouts[oHtml.getAttribute("id")];
...
info.control.stop=function(){if(info.control.state==apf.tween.STOPPED){return;}info.control.state=apf.tween.STOPPING;
clearQueue(oHtml);if(info.onstop){info.onstop(oHtml,info.userdata);}};}var tweens=info.tweens,stepFunction=function(step){if(info.control&&info.control.state){info.control.state=apf.tween.STOPPED;
...
}return apf.tween.easeOutBounce(t*2-1,0,dx)*0.5+dx*0.5+x_min;}};})(apf);apf.xmldb=new (function(){var _self=this;
this.xmlDocTag="a_doc";this.xmlIdTag="a_id";this.xmlListenTag="a_listen";this.htmlIdTag="id";
this.disableRDB=false;this.$xmlDocLut=[];this.$nodeCount={};var cleanRE=/(?:a_doc|a_id|a_listen|a_loaded)=(?:"|')[^'"]+(?:"|')/g,whiteRE=/>[\s\n\r\t]+</g;
this.garbageCollect=function(){var xmlNode,cache=apf.xmldb.$xmlDocLut,docId,model;
for(var i=0,l=cache.length;i<l;i++){xmlNode=cache[i];if(!xmlNode||xmlNode.nodeFunc){continue;
}docId=i;model=apf.nameserver.get("model",docId);if(!model||model.data!=xmlNode){cache[i]=null;
...
if(!model){return;}if(model.$propBinds[sUId[1]][sUId[2]]){if(!apf.isChildOf(model.data,xmlNode,true)){return false;
}var xpath=model.$propBinds[sUId[1]][sUId[2]].listen;var node=xpath?apf.queryNode(model.data,xpath):xmlNode;
...
if(amlNode){amlNode.$xmlUpdate.apply(amlNode,args);}};}}if(!listen||listen.indexOf(";"+id+";")==-1){xmlNode.setAttribute(this.xmlListenTag,(listen?listen+id:";"+id)+";");
...
delete this.$listeners[id];}else{id="e"+o.$uniqueId;}for(var newnodes=[],i=0;i<nodes.length;
i++){if(nodes[i]!=id){newnodes.push(nodes[i]);}}xmlNode.setAttribute(this.xmlListenTag,newnodes.join(";"));
return xmlNode;};this.setTextNode=apf.setTextNode=function(pNode,value,xpath,undoObj,range){var tNode;
...
}if(oldNode.ownerDocument.importNode&&newNode.ownerDocument!=oldNode.ownerDocument){var oldNodeS=xmlNode;
newNode=oldNode.ownerDocument.importNode(newNode,true);if(oldNodeS.parentNode){oldNodeS.parentNode.removeChild(oldNodeS);
...
}if(unique&&pNode.selectSingleNode(xmlNode.tagName)){return false;}if(pNode.ownerDocument.importNode&&pNode.ownerDocument!=xmlNode.ownerDocument){var oldNode=xmlNode;
xmlNode=pNode.ownerDocument.importNode(xmlNode,true);if(oldNode.parentNode){oldNode.parentNode.removeChild(oldNode);
...
}}}}else{if(xmlNode.parentNode){this.removeNode(xmlNode);}}if(undoObj&&!undoObj.$filled){undoObj.$filled=true;
this.cleanNode(xmlNode);}else{this.cleanNode(xmlNode);}pNode.insertBefore(xmlNode,beforeNode);
...
if(pNode.ownerDocument.importNode&&pNode.ownerDocument!=xmlNode.ownerDocument){var oldNode=xmlNode;
xmlNode=pNode.ownerDocument.importNode(xmlNode,true);if(oldNode.parentNode){oldNode.parentNode.removeChild(oldNode);
...
}var requestedWithParam=apf.config?apf.config["requested-with-getparam"]:null;if(requestedWithParam){httpUrl+=(httpUrl.indexOf("?")==-1?"?":"&")+requestedWithParam+"=1";
...
}if(!requestedWithParam){setRequestHeader("X-Requested-With","XMLHttpRequest");
}if(!options.headers||!options.headers["Content-type"]){setRequestHeader("Content-type",options.contentType||this.contentType||(this.useXML||options.useXML?"text/xml":"text/plain"));
...
this.preserveWhiteSpace=false;this.$waitQueue={};this.$callCount=0;var RE=[/\<\!(DOCTYPE|doctype)[^>]*>/,/&nbsp;/g,/<\s*\/?\s*(?:\w+:\s*)[\w-]*[\s>\/]/g],XPATH="//@*[not(contains(local-name(), '.')) and not(translate(local-name(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = local-name())]";
...
});if(!this.supportNamespaces){str=str.replace(/xmlns\=\"[^"]*\"/g,"");}var xmlNode=apf.getXmlDom(str);
if(apf.xmlParseError){apf.xmlParseError(xmlNode);}xmlNode=xmlNode.documentElement;
...
var rest=p?n.splice(p,n.length-p):[];}}this.$parseContext=[amlNode,options];this.$addParseState(amlNode,options||{});
var nodes,nodelist={},prios=[],_self=this;var recur;(recur=function(amlNode,nodes){var cL,newNode,node,nNodes,cNodes=amlNode.childNodes,i=0,l=nodes.length;
...
}}if(this.$waitQueue[amlNode.$uniqueId]&&this.$waitQueue[amlNode.$uniqueId].$shouldWait){return(docFrag||doc);
}if(options.timeout){$setTimeout(function(){_self.$continueParsing(amlNode,options);
});}else{this.$continueParsing(amlNode,options);}return(docFrag||doc);};this.$isPaused=function(amlNode){return this.$waitQueue[amlNode.$uniqueId]&&this.$waitQueue[amlNode.$uniqueId].$shouldWait>0;
};this.$addParseState=function(amlNode,options){var waitQueue=this.$waitQueue[amlNode.$uniqueId]||(this.$waitQueue[amlNode.$uniqueId]=[]);
...
if(amlNode.$parseOptions){var prios=amlNode.$parseOptions.prios,nodelist=amlNode.$parseOptions.nodelist,i,j,l,l2,node;
delete amlNode.$parseOptions;prios.sort();for(i=0,l=prios.length;i<l;i++){var nodes=nodelist[prios[i]];
...
}(function _recur(nodes){var node,nNodes;for(var i=0,l=nodes.length;i<l;i++){if(!(node=nodes[i]).$amlLoaded){node.dispatchEvent("DOMNodeInsertedIntoDocument");
...
}};this.$createNode=function(doc,nodeType,xmlNode,namespaceURI,nodeName,nodeValue){var o;
...
o.tagName=prefix?prefix+":"+nodeName:nodeName;if(xmlNode){if((id=xmlNode.getAttribute("id"))&&!self[id]){o.$propHandlers.id.call(o,o.id=id);
}var attr=xmlNode.attributes,n;for(var a,na,i=0,l=attr.length;i<l;i++){o.attributes.push(na=new apf.AmlAttr(o,(n=(a=attr[i]).nodeName),a.nodeValue));
if(n=="render"){o.render=true;}else{if(n.substr(0,2)=="on"){na.$triggerUpdate();
}}}}break;case 2:o=new apf.AmlAttr();o.name=o.nodeName=nodeName;if(nodeValue||(nodeValue=xmlNode&&xmlNode.nodeValue)){o.value=o.nodeValue=nodeValue;
...
}else{this.documentElement=amlNode;}}if(amlNode==beforeNode){return amlNode;}if(this==amlNode){throw new Error(apf.formatErrorString(0,this,"Insertbefore DOM operation","Cannot append node as a child of itself."));
...
}if(amlNode.parentNode){amlNode.removeNode(isMoveWithinParent,true);}amlNode.parentNode=this;
...
if(document.adoptNode&&amlNode.$ext&&amlNode.$ext.nodeType==1){var reappendlist=[];
var iframelist=apf.getArrayFromNodelist(amlNode.$ext.getElementsByTagName("iframe"));
if(amlNode.$ext.tagName=="IFRAME"){document.adoptNode(amlNode.$ext);}for(var i=0;
i<iframelist.length;i++){reappendlist[i]=[iframelist[i].parentNode,iframelist[i].nextSibling,document.adoptNode(iframelist[i]),];
...
for(var i=reappendlist.length-1;i>=0;i--){reappendlist[i][0].insertBefore(reappendlist[i][2],reappendlist[i][1]);
}reappendlist=[];}amlNode.dispatchEvent("DOMNodeInserted",{$beforeNode:beforeNode,relatedNode:_self,$isMoveWithinParent:isMoveWithinParent,$oldParentHtmlNode:oldParentHtmlNode,$oldParent:oldParent,bubbles:true});
if(initialAppend&&!noHtmlDomEdit&&beforeNode&&amlNode.$ext&&!amlNode.$coreHtml){nextNode=beforeNode;
while(nextNode&&!(nextNode.$altExt||nextNode.$ext)){nextNode=nextNode.nextSibling;
}amlNode.$pHtmlNode.insertBefore(amlNode.$altExt||amlNode.$ext,nextNode&&(nextNode.$altExt||nextNode.$ext)||null);
for(var i=reappendlist.length-1;i>=0;i--){reappendlist[i][0].insertBefore(reappendlist[i][2],reappendlist[i][1]);
}}}var doc=this.nodeType==this.NODE_DOCUMENT?this:this.ownerDocument;if(!doc||doc.$domParser.$isPaused(this)){return amlNode;
}if(this.nodeType==this.NODE_DOCUMENT_FRAGMENT||!this.$amlLoaded){return amlNode;
}if(initialAppend&&!amlNode.render){(this.ownerDocument||this).$domParser.$continueParsing(amlNode,{delay:true});
...
}return $init.call(this,tagName,nodeFunc,struct);};this.$init(function(tagName,nodeFunc,struct){this.$events={};
this.$inheritProperties={};this.attributes=new apf.AmlNamedNodeMap(this);this.nodeFunc=nodeFunc;
this.localName=tagName;if(struct){var nodes,prop,i,l,attr;if(struct.childNodes){nodes=struct.childNodes;
delete struct.childNodes;}for(prop in struct){if(prop=="htmlNode"){continue;}attr=new apf.AmlAttr(this,prop,struct[prop]);
if(prop=="id"){this.$propHandlers.id.call(this,this.id=struct.id);}else{if(prop=="hotkey"){this.$propHandlers.hotkey.call(this,this.hotkey=struct.hotkey);
}else{if(prop.substr(0,2)=="on"){attr.$triggerUpdate();}}}this.attributes.push(attr);
...
}if(self[this.name]==this){self[this.name]=null;apf.nameserver.remove(this.localName,this);
apf.nameserver.remove("all",this);}if(self[value]){console.warn("ID collision of APF element: '"+value+"'");
}if(!self[value]||!self[value].hasFeature){try{self[value]=this;}catch(ex){}}apf.nameserver.register(this.localName,value,this);
apf.nameserver.register("all",value,this);this.name=value;}};this.$booleanProperties={};
...
if(!this.$amlLoaded&&name!="id"&&name!="hotkey"){return;}if(noTrigger){a.$setValue(value);
}else{a.dispatchEvent("DOMNodeInsertedIntoDocument",{relatedNode:this});a.dispatchEvent("DOMNodeInserted",{relatedNode:this,bubbles:true});
}return;}var oldValue=a.nodeValue;a.$setValue(value);if(noTrigger||!this.$amlLoaded){return;
}a.$triggerUpdate(null,oldValue);};this.setAttributeNode=function(attrNode){this.attributes.setNamedItem(attrNode);
...
}},true);this.addEventListener("DOMNodeInsertedIntoDocument",function(e){this.$amlLoaded=true;
});}).call(apf.AmlElement.prototype=new apf.AmlNode());apf.AmlCharacterData=function(){this.data="";
this.length=0;this.$init(true);this.appendData=function(sValue){this.dispatchEvent("DOMCharacterDataModified",{value:sValue});
...
this.nodeName="#text";this.serialize=function(){return apf.escapeXML(this.nodeValue);
...
this.MODIFICATION=1;this.ADDITION=2;this.REMOVAL=3;this.serialize=this.toString=function(){return this.name+'="'+apf.escapeXML(String(this.value))+'"';
...
this.$lastValue=value;};this.$triggerUpdate=function(e,oldValue){var name=this.name,value=this.value||this.nodeValue,host=this.ownerElement,isEvent=name.substr(0,2)=="on";
if(!this.specified){if(isEvent&&this.$lastValue==value||name=="id"&&host.id){this.specified=true;
return;}}if(isEvent){if(host.$events[name]){host.removeEventListener(name.substr(2),host.$events[name]);
}if(value){host.addEventListener(name,(host.$events[name]=(typeof value=="string"?new Function("event",value):value)));
...
};this.querySelector=function(){};this.querySelectorAll=function(){};this.hasFocus=function(){};
...
apf.AmlTextRectangle=function(host){var _self=this;function handler(){var pos=_self.getAbsolutePosition(_self.$ext);
...
apf.xhtml.setElement("pre",apf.XhtmlSkipChildrenElement);apf.xinclude=new apf.AmlNamespace();
apf.setNamespace("http://www.w3.org/2001/XInclude",apf.xinclude);apf.XiInclude=function(struct,tagName){this.$init(tagName||"include",apf.NODE_HIDDEN,struct);
...
var domParser=this.ownerDocument.$domParser;if(!this.defer){domParser.$pauseParsing.apply(domParser,this.$parseContext=domParser.$parseContext||[this.parentNode]);
}loadIncludeFile.call(this,this.$path);};function done(xmlNode){if(this.callback){this.callback({xmlNode:xmlNode,amlNode:this.parentNode,addedNode:this.previousSibling||this.nextSibling});
...
}};if(domParser.$continueParsing(this.$parseContext[0])===false){var o2=(domParser.$parseContext[1]||(domParser.$parseContext[1]={})),cb2=o.callback;
o2.callback=function(){if(cb){cb.call(_self.ownerDocument);}domParser.$continueParsing(_self.$parseContext[0]);
};}}else{done.call(this,xmlNode);}}else{if(!this.defer){domParser.$continueParsing(this.$parseContext[0]);
...
};function visibleHandler(e){if(!(this.$rule_header||this.$rule_v||this.$rule_h)||!this.parentNode){return;
...
this.$focussable=apf.KEYBOARD_MOUSE;this.visible=2;this.minwidth=1;this.minheight=1;
this.$booleanProperties["disable-keyboard"]=true;this.$booleanProperties.visible=true;
...
this.addEventListener("DOMNodeInsertedIntoDocument",this.$updateDisabled||(this.$updateDisabled=function(e){apf.GuiElement.propHandlers.disabled.call(_self,_self.disabled);
...
},"disable-keyboard":function(value){this.disableKeyboard=apf.isTrue(value);},tooltip:function(value){this.$ext.setAttribute("title",(value||"")+(this.hotkey?" ("+(apf.isMac?apf.hotkeys.toMacNotation(this.hotkey):this.hotkey)+")":""));
...
if(this.$canLeechSkin&&!this.skin&&(skinName=this.$canLeechSkin.dataType==apf.STRING?this.$canLeechSkin:this.localName)&&pNode&&pNode.$originalNodes&&(skinNode=pNode.$originalNodes[skinName])&&skinNode.getAttribute("inherit")){var link=skinNode.getAttribute("link");
...
while(p){if(p.show){p.show();}p=p.parentNode;}};this.validate=function(ignoreReq,nosetError,force){if(force||!this.isValid(!ignoreReq)&&!nosetError){this.setError();
...
this.showMe();if(this.invalidmsg||value){errBox.display(this);}if(apf.document.activeElement&&apf.document.activeElement!=this){this.focus(null,{mouse:true});
...
};this.$propHandlers.required=this.$propHandlers.custom=this.$propHandlers.min=this.$propHandlers.max=this.$propHandlers.maxlength=this.$propHandlers.minlength=this.$propHandlers.notnull=this.$propHandlers.checkequal=function(value,prop){(this.$vOptions||(this.$vOptions={}))[prop]=value;
...
if(found&&!this.allowMultipleErrors){return true;}}}return found;}this.validate=this.isValid=function(ignoreReq,nosetError,page){var found=checkValidChildren.call(this,page||this,ignoreReq,nosetError);
...
return handler?handler.call(this,xmlNode,callback):defaultValue||"";};this.$hasBindRule=function(name){return this.$attrBindings[name]||this.$bindings&&this.$bindings[name];
...
}this.$loadqueue=null;if(this.dataParent&&this.dataParent.xpath){this.dataParent.parent.signalXmlUpdate[this.$uniqueId]=!xmlNode;
...
this.$isLoading=true;apf.xmldb.applyChanges("insert",insertPoint);this.$isLoading=false;
if(this.selectable&&this.autoselect){if(this.xmlNode==newNode){this.$selectDefault(this.xmlNode);
...
if(list[modelId]==-1){continue;}xpath=paths[i+1];if(modelId=="#"||xpath=="#"){var m=(rule.cvalue3||(rule.cvalue3=apf.lm.compile(rule.value,{xpathmode:5}))).call(this,this.xmlRoot);
if(typeof m!="string"){model=m.model&&m.model.$isModel&&m.model;if(model){xpath=m.xpath;
}else{if(m.model){model=typeof m.model=="string"?apf.xmldb.findModel(m.model):m.model;
xpath=apf.xmlToXpath(m.model,model.data)+(m.xpath?"/"+m.xpath:"");}else{this.addEventListener("prop."+prop,function(e){var m=(rule.cvalue3||(rule.cvalue3=apf.lm.compile(rule.value,{xpathmode:5}))).call(this,this.xmlRoot);
...
};this.getSortSettings=function(){return this.$sort.get();};this.$propHandlers.simpledata=function(value){if(value){this.getTraverseNodes=function(xmlNode){if(this.$sort&&!this.$isLoading){var nodes=apf.getArrayFromNodelist((xmlNode||this.xmlRoot).childNodes);
return this.$sort.apply(nodes);}return(xmlNode||this.xmlRoot).childNodes;};this.getFirstTraverseNode=function(xmlNode){return this.getTraverseNodes()[0];
};this.getLastTraverseNode=function(xmlNode){var nodes=this.getTraverseNodes();
...
}};this.$load=function(XMLRoot){apf.xmldb.addNodeListener(XMLRoot,this);this.$isLoading=true;
var length=this.getTraverseNodes(XMLRoot).length;if(!this.renderRoot&&!length){return this.clear(null,null,true);
}var nodes=this.$addNodes(XMLRoot,null,null,this.renderRoot,null,0,"load");this.$fill(nodes);
this.$isLoading=false;if(this.selectable){if(this["default"]){this.select(this["default"]);
...
}if(length!=this.length){this.setProperty("length",length);}};var actionFeature={insert:127,replacenode:127,attribute:255,add:251,remove:110,"redo-remove":79,synchronize:127,"move-away":297,move:141};
...
}if(action=="redo-remove"){var loc=[xmlNode.parentNode,xmlNode.nextSibling];lastParent.appendChild(xmlNode);
var eachNode=this.isTraverseNode(xmlNode);if(loc[0]){loc[0].insertBefore(xmlNode,loc[1]);
}else{lastParent.removeChild(xmlNode);}if(!eachNode){xmlNode=lastParent;}}do{if(action=="add"&&this.isTraverseNode(xmlNode)&&startNode==xmlNode){break;
...
}if(parentHTMLNode||this.$isTreeArch&&pNode==this.xmlRoot){parentHTMLNode=(this.$findContainer&&parentHTMLNode&&parentHTMLNode.nodeType==1?this.$findContainer(parentHTMLNode):parentHTMLNode)||this.$container;
...
if(this.$selectTimer.nextNode==this.xmlRoot&&!this.renderRoot){this.$selectTimer.nextNode=null;
...
}if(action=="remove"){length--;}if(length!=this.length){this.setProperty("length",length);
}}if(this.signalXmlUpdate&&actionFeature[action]&16){var uniqueId;for(uniqueId in this.signalXmlUpdate){if(parseInt(uniqueId)!=uniqueId){continue;
...
if(value.match(/^\w+::/)){var model=value.split("::");if(!apf.xPathAxis[model[0]]){this.setProperty("model",model[0]);
...
xmlNode=model.data;}else{if(m.model){model=apf.xmldb.findModel(m.model);node=m.model.selectSingleNode(m.xpath);
xmlNode=m.model;}else{}}sel[1]=m.xpath;}else{model=sel[0]&&apf.nameserver.get("model",sel[0])||this.$model,node=model?model.queryNode(sel[1]):(xmlNode||this.xmlRoot).selectSingleNode(sel[1]);
...
}else{atAction="setValueByXpath";xpath=sel[1];if(!this.$createModel||this.getModel()&&!this.getModel().$createModel){throw new Error("Model data does not exist, and I am not allowed to create the element for xpath '"+xpath+"' and element "+this.serialize(true));
...
}var nodes=this.getTraverseNodes();if(nodes.length){if(this["default"]){this.select(this["default"]);
}else{if(this.autoselect){this.select(nodes[0],null,null,null,true);}}}else{if(this.clearSelection){this.clearSelection();
}}if(!nodes.length){this.$removeClearMessage();this.$setClearMessage(this["empty-message"],"empty");
}if(nodes.length!=this.length){this.setProperty("length",nodes.length);}return false;
}});this.addEventListener("$clear",function(){if(!this.caching){return;}if(this.hasFeature(apf.__MULTISELECT__)&&this.$subTreeCacheContext&&this.$subTreeCacheContext.oHtml){if(this.renderRoot){this.$subTreeCacheContext.parentNode.insertBefore(this.$subTreeCacheContext.oHtml,this.$subTreeCacheContext.beforeNode);
...
}}catch(e){}r.select();};this.$txt.onblur=function(){if(this.host.$autocomplete){return;
...
}else{this.clear();}};this.$keyHandler=function(e){var key=e.keyCode,ctrlKey=e.ctrlKey,shiftKey=e.shiftKey,selHtml=this.$caret||this.$selected;
...
}this.remove();break;case 36:var node=this.getFirstTraverseNode();if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node);
}this.select(node,false,shiftKey);this.$container.scrollTop=0;break;case 35:var node=this.getLastTraverseNode();
if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node,true);
}this.select(node,false,shiftKey);this.$container.scrollTop=this.$container.scrollHeight;
...
}else{return;}if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node);
}selHtml=apf.xmldb.findHtmlNode(node,this);if(selHtml.offsetTop<oExt.scrollTop){oExt.scrollTop=Array.prototype.indexOf.call(this.getTraverseNodes(),node)<items?0:selHtml.offsetTop-margin[0];
...
}else{return;}if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node,true);
}selHtml=apf.xmldb.findHtmlNode(node,this);if(selHtml.offsetTop+selHtml.offsetHeight>oExt.scrollTop+oExt.offsetHeight){oExt.scrollTop=selHtml.offsetTop-oExt.offsetHeight+selHtml.offsetHeight+margin[0];
...
}if(node){this.$setTempSelected(node,ctrlKey,shiftKey,true);}else{return;}if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node);
}selHtml=apf.xmldb.findHtmlNode(node,this);if(selHtml.offsetTop<oExt.scrollTop){oExt.scrollTop=Array.prototype.indexOf.call(this.getTraverseNodes(),node)<items?0:selHtml.offsetTop-margin[0];
...
}if(node){this.$setTempSelected(node,ctrlKey,shiftKey);}else{return;}if(this.hasFeature(apf.__VIRTUALVIEWPORT__)){return this.$viewport.scrollIntoView(node,true);
}selHtml=apf.xmldb.findHtmlNode(node,this);if(selHtml.offsetTop+selHtml.offsetHeight>oExt.scrollTop+oExt.offsetHeight){oExt.scrollTop=selHtml.offsetTop-oExt.offsetHeight+selHtml.offsetHeight+margin[0];
...
}}}var elCaption=this.$getLayoutNode("item","caption",htmlNode);if(elCaption){if(elCaption.nodeType==1){elCaption.innerHTML=this.$applyBindRule("caption",xmlNode);
...
}if(elIcon){if(elIcon.nodeType==1){elIcon.setAttribute("style","background-image:url("+apf.getAbsolutePath(this.iconPath,this.$applyBindRule("icon",xmlNode))+")");
}else{elIcon.nodeValue=apf.getAbsolutePath(this.iconPath,this.$applyBindRule("icon",xmlNode));
...
}}}}if(elCaption){apf.setNodeValue(elCaption,this.$applyBindRule("caption",xmlNode));
...
}if(this.activepage==(page.name||page)){return callback&&callback(this.getPage(page));
}this.$lastCallback=callback;this.setProperty("activepage",page);};this.$supportedProperties.push("activepage","activepagenr","length","src","loading","trans-in","trans-out");
...
if(this.$activepage){this.$activepage.$deactivate();}page.$activate();this.$activepage=page;
if(this.hideLoader){if(page.$rendered!==false){this.hideLoader();}else{page.addEventListener("afterrender",function(){this.parentNode.hideLoader();
...
this.$scroll=!this.$scale;this.$order=value.indexOf("order")>-1;if(this.$scale){this.$maxBtnWidth=parseInt(this.$getOption("button","maxwidth"))||150;
...
}};function visCheck(){scalersz.call(this);}this.anims="add|remove|sync";this.$scaleinit=function(node,type,callback,force){var pg=this.getPages();
...
if(force&&!this.$ext.offsetWidth&&!this.$ext.offsetHeight||this.anims.indexOf(type)==-1){scalersz.call(this);
if(type=="add"){node.dispatchEvent("afteropen");}else{if(type=="remove"){node.dispatchEvent("afterclose");
}}return false;}if(!apf.window.vManager.check(this,"tabscale",visCheck)){return;
}if(!type){return scalersz.call(this);}if(this.$control&&this.$control.type!="remove"&&this.$control.stop){this.$control.stop();
...
}}}};function btnMoHandler(e){var pos=apf.getAbsolutePosition(this);if(e.clientX<=pos[0]||e.clientY<=pos[1]||e.clientX>=pos[0]+this.offsetWidth||e.clientY>=pos[1]+this.offsetHeight){apf.removeListener(_self.$buttons,"mouseout",btnMoHandler);
if(_self.$control.state==apf.tween.STOPPED){delete _self.$waitForMouseOut;_self.$scaleinit(null,"sync");
}else{if(_self.$waitForMouseOut){_self.$waitForMouseOut=2;}}}}this.$control.type=type;
if(type=="add"){var htmlNode=node.$button;htmlNode.style.width=this.$minBtnWidth+"px";
...
if(isLast){this.$buildScaleAnim(anim,pg,node);}if(this.nextTabInLine){this.set(this.nextTabInLine);
delete this.nextTabInLine;}else{if(this.$activepage==node){var ln=node.nextSibling;
...
}}}this.$waitForMouseOut=true;if(!isLast){apf.addListener(_self.$buttons,"mouseout",btnMoHandler);
...
}this.insertBefore(page,before);return page;};this.remove=function(nameOrId,force,noAnimation){var page=typeof nameOrId=="object"?nameOrId:this.$findPage(nameOrId);
if(!page){return false;}var e={page:page};if(typeof force=="object"){e.htmlEvent=force;
force=false;}if(!force&&this.dispatchEvent("close",e)===false){return;}if(this.$scale&&!noAnimation){this.$scaleinit(page,"remove",function(){page.destroy(true,true);
},true);}else{if(page.dispatchEvent("afterclose")!==false){page.destroy(true,true);
}}return page;};function getButtonsWidth(){var cId="cache_"+this.$buttons.childNodes.length;
if(SCROLLANIM[cId]){return SCROLLANIM[cId];}var iWidth=0;for(var i=0,l=this.$buttons.childNodes.length;
i<l;i++){if(typeof this.$buttons.childNodes[i].offsetWidth!="undefined"){iWidth+=this.$buttons.childNodes[i].offsetWidth;
...
}var bAnimating=true;if(typeof dir=="undefined"){dir=SCROLL_LEFT;}var iCurrentLeft=this.$buttons.offsetLeft,size=e.delta?Math.round(e.delta*36):SCROLLANIM.size,iBoundary=getAnimationBoundary.call(this,dir),_self=this;
...
if(e.$doOnlyAdmin||e.relatedNode!=this||amlNode.localName!="page"){return;}if((this.activepage||this.activepage==0)&&this.activepage!=-1){if(this.nextTabInLine){this.set(this.nextTabInLine);
}if(!this.nextTabInLine&&this.$activepage==amlNode){var ln=amlNode.nextSibling;
...
}if(this.lastChild==amlNode&&rn){rn&&rn.$last();}if(ln||rn){this.set(ln||rn);}else{amlNode.$deactivate();
this.$activepage=this.activepage=this.activepagenr=null;this.setProperty("activepage",null);
}}else{if(this.$scale){this.$scaleinit();}}delete this.nextTabInLine;}this.setProperty("length",this.getPages().length-1);
...
}var pages=this.getPages();if(!e.$beforeNode){var lastChild,pg=pages;if(lastChild=pg[pg.length-2]){lastChild.$last(true);
}amlNode.$last();}var p2,p=pages[0];if(amlNode==p){if(p2=this.getPage(1)){p2.$first(true);
...
}}else{if(!this.activepage&&!this.$activepage&&!amlNode.render||amlNode.$rendered){this.set(amlNode);
}}if(this.$scale&&amlNode.visible&&!e.$isMoveWithinParent){this.$scaleinit(amlNode,"add");
}else{amlNode.dispatchEvent("afteropen");}this.setProperty("length",this.getPages().length);
...
}}else{this.isPages=false;}this.setProperty("length",this.getPages().length);this.ready=true;
if(!this.activepage&&this.getAttribute("src")){this.src=this.getAttribute("src");
...
this.$booleanProperties.dragroot=true;this.$booleanProperties.disableremove=true;
this.$supportedProperties.push("openadd","startcollapsed","nocollapse","singleopen","prerender","removecontainer","animation","dragroot","disableremove");
this.openadd=true;this.startcollapsed=1;this.prerender=true;this.disableremove=false;
this.expandAll=function(){if(!this.xmlRoot){return;}var xpath=this.each.split("|").join("["+this.each.replace(/\|/g," or ")+"]|.//"),_self=this;
...
})(xmlNode)!==false){this.select(xmlNode);}};this.expandList=function(paths,onFinished){var _self=this;
var root=this.xmlRoot;function expand(currentSelector,allSelectors){var selectorNode=root.selectSingleNode(currentSelector);
if(!selectorNode){return hasExpanded(currentSelector);}_self.slideToggle(apf.xmldb.getHtmlNode(selectorNode,_self),1,true,null,function(){var timesRan=0;
var callback=function(){if(!_self.$hasLoadStatus(root.selectSingleNode(currentSelector),"loaded")){if(++timesRan<3){return setTimeout(function(){_self.slideToggle(apf.xmldb.getHtmlNode(selectorNode,_self),1,true,null,callback);
},1000);}else{var allAncestors=allSelectors.filter(function(s){return s.indexOf(currentSelector+"/")===0;
});hasExpanded(currentSelector);allAncestors.forEach(function(sel){hasExpanded(sel);
});return null;}}hasExpanded();var childSelectors=allSelectors.filter(function(s){return s.indexOf(currentSelector+"/")===0&&currentSelector.split("/").length+1===s.split("/").length;
});childSelectors.forEach(function(selector){expand(selector,allSelectors);});};
callback();});}var expandCount=0;function hasExpanded(){if(++expandCount===paths.length){onFinished();
}}var rootNodes=paths.filter(function(p){return p.split("/").length===1;});rootNodes.forEach(function(node){expand(node,paths);
});};this.selectPath=function(path){};this.slideToggle=function(htmlNode,force,immediate,userAction,callback){callback=typeof callback==="function"?callback:function(){};
if(this.nocollapse||userAction&&this.disabled){return callback();}if(!htmlNode){htmlNode=this.$selected;
}if(!htmlNode){return callback();}var id=htmlNode.getAttribute(apf.xmldb.htmlIdTag);
while(!id&&htmlNode.parentNode){id=(htmlNode=htmlNode.parentNode).getAttribute(apf.xmldb.htmlIdTag);
}var container=this.$getLayoutNode("item","container",htmlNode);if(!container){return callback();
...
}if((!container.childNodes.length||container.firstChild.nodeType==3&&container.firstChild.nodeValue.trim()=="")&&!this.$getBindRule("insert",xmlNode)&&!this.getTraverseNodes(xmlNode).length){return callback&&callback();
...
_self.dispatchEvent("collapse",{xmlNode:xmlNode});}});};this.$isStartCollapsed=function(xmlNode){return this.$hasBindRule("collapsed")?(this.$getDataNode("collapsed",xmlNode)?true:false):(this.$hasBindRule("expanded")?(this.$getDataNode("expanded",xmlNode)?false:true):this.startcollapsed);
};this.$add=function(xmlNode,Lid,xmlParentNode,htmlParentNode,beforeNode,isLast,depth,nextNode,action){if(this.$isTreeArch&&this.$needsDepth&&typeof depth=="undefined"){var loopNode=xmlParentNode;
...
}}if(htmlParentNode.style){if(this.openadd&&htmlParentNode!=this.$container&&htmlParentNode.style.display!="block"){if(!this.$isStartCollapsed(xmlParentNode)){this.slideOpen(htmlParentNode,xmlParentNode,true);
...
}}}var isClosed=hasChildren&&apf.getStyle(container,"display")=="none";isLast=this.getNextTraverse(xmlNode,null,oneLeft?2:1)?false:true,state=(hasChildren?HAS_CHILD:0)|(isClosed?IS_CLOSED:0)|(isLast?IS_LAST:0);
...
}var rule=this.$getBindRule("insert",xmlNode),_self=this,xmlContext=rule&&rule.match?(rule.cmatch||rule.compile("match"))(xmlNode):xmlNode;
...
},100);if(rule.get){this.getModel().$insertFrom(rule.getAttribute("get"),{xmlNode:xmlContext,insertPoint:xmlContext,amlNode:this,callback:function(data,state,extra){if(state!=apf.SUCCESS){_self.$setLoadStatus(xmlNode,"potential");
_self.$removeLoading(xmlNode);_self.slideToggle(apf.xmldb.getHtmlNode(xmlNode,_self),2,true);
}if(callback){callback(data,state,extra);}}});}else{if(this.$applyBindRule("insert",xmlNode)){this.insert(data,{insertPoint:xmlContext});
...
this.select(this.$tempsel);this.$tempsel=null;this.timer=null;}});this.scrollIntoView=function(sNode,onTop){var selHtml=apf.xmldb.getHtmlNode(sNode,this),top;
if(!selHtml){return;}top=apf.getAbsolutePosition(selHtml,this.$container)[1];if(onTop){if(top<=this.$container.scrollTop){this.$container.scrollTop=top;
}}else{if(top>this.$container.scrollTop+this.$container.offsetHeight){this.$container.scrollTop=top-this.$container.offsetHeight+selHtml.offsetHeight;
}}};this.addEventListener("keydown",function(e){var key=e.keyCode,ctrlKey=e.ctrlKey,shiftKey=e.shiftKey,selHtml=this.$caret||this.$selected,pos,top,el,node,nodes,sNode,pNode,container;
if(e.returnValue==-1||!selHtml||this.renaming){return;}var selXml=this.caret||this.selected,oExt=this.$container;
switch(key){case 13:if(this.$tempsel){this.$selectTemp();}if(this.ctrlselect=="enter"){this.select(this.caret,true);
}this.choose(selHtml);break;case 32:if(this.$tempsel){this.$selectTemp();}if(ctrlKey||!this.isSelected(this.caret)){this.select(this.caret,true);
}return false;case 46:if(this.disableremove){return;}if(this.$tempsel){this.$selectTemp();
}this.remove();break;case 36:this.$setTempSelected(this.getFirstTraverseNode(),false,shiftKey);
...
sNode=this.getNextTraverse(node,true);if(sNode&&sNode!=node){nodes=this.getTraverseNodes(sNode);
...
};this.$selectDefault=function(xmlNode){var firstNode=this.getFirstTraverseNode(xmlNode);
if(!firstNode){return;}if(this.select(firstNode,null,null,null,true)){return true;
...
}if(tgt&&!apf.isChildOf(data,tgt,true)){return[tgt,rule];}}return false;};this.$dragDrop=function(xmlReceiver,xmlNodeList,rule,defaction,isParent,srcRule,event,forceCopy){if(action=="tree-append"&&isParent){return false;
}var action=rule&&rule.action;if(action){action=(rule.caction||rule.compile("action"))(xmlNodeList[0]);
}else{action=defaction;}if(!event){event={};}var context={internal:apf.DragServer.dragdata&&apf.DragServer.dragdata.host==this,ctrlKey:event.ctrlKey,keyCode:event.keyCode},ifcopy=rule&&rule.copy;
if(typeof forceCopy=="boolean"){ifcopy=forceCopy;}else{if(ifcopy){context.event=event||{};
...
}}}}}}if(!ifcopy&&srcRule){for(var i=0,l=srcRule.length;i<l;i++){ifcopy=typeof srcRule[i]=="object"&&srcRule[i].copy?!apf.isFalse((srcRule[i].ccopy||srcRule[i].compile("copy"))(xmlNodeList[0],context)):event.ctrlKey;
...
},stop:function(runEvent,success,e){if(this.last){this.dragout();}this.dragdata.host.dispatchEvent("dragstop",apf.extend(this.dragdata,{success:success}));
...
if(!apf.DragServer.dragdata.started&&Math.abs(apf.DragServer.coordinates.clientX-c.clientX)<6&&Math.abs(apf.DragServer.coordinates.clientY-c.clientY)<6){apf.DragServer.stop(true,null,e);
...
}var success=apf.DragServer.dragdrop(host,el,apf.DragServer.dragdata.host,e);apf.DragServer.stop(true,success,e);
...
}else{apf.window.$focus(this,e);}return this;}if(this.$focus&&!this.editable&&(!e||!e.mouse||this.$focussable==apf.KEYBOARD_MOUSE)){this.$focus(e);
...
}if(o.interactive&1){return;}o.interactive=(o.interactive||0)+1;};this.$propHandlers.resizable=function(value){if(apf.isFalse(value)){this.resizable=false;
}else{if(apf.isTrue(value)){this.resizable="true";}}this.$ext.style.cursor="";var o=this.oResize||this.$ext;
if(o.interactive&2){return;}if(!_self.editable){apf.addListener(o,"mousedown",function(){resizeStart.apply(o,arguments);
...
}dragOutline=false;if(_self.dispatchEvent("beforedragstart",{htmlEvent:e})===false){return;
...
nX=pos[0]-(oX=e.clientX);nY=pos[1]-(oY=e.clientY);if(!(reparent||(oOutline&&oOutline.self))){if(_self.$ext.style.right){_self.$ext.style.left=pos[0]+"px";
_self.$ext.style.right="";}if(_self.$ext.style.bottom){_self.$ext.style.top=pos[1]+"px";
_self.$ext.style.bottom="";}}document.onmousemove=dragMove;document.onmouseup=function(e,cancel){document.onmousemove=document.onmouseup=null;
...
}resizeOutline=false;var ext=_self.$ext;if(!resizeOutline){var diff=apf.getDiff(ext);
hordiff=diff[0];verdiff=diff[1];}startPos=apf.getAbsolutePosition(ext);startPos.push(ext.offsetWidth);
startPos.push(ext.offsetHeight);myPos=apf.getAbsolutePosition(ext,ext.offsetParent,true);
startGeo=[ext.style.left,ext.style.top,ext.style.right,ext.style.bottom,ext.style.width,ext.style.height];
...
}var iMarginLeft;if(ext.style.right){ext.style.left=myPos[0]+"px";}if(ext.style.bottom){ext.style.top=myPos[1]+"px";
}if(!options||!options.nocursor){if(lastCursor===null){lastCursor=document.body.style.cursor;
}document.body.style.cursor=getCssCursor(resizeType)+"-resize";}document.onmousemove=resizeMove;
...
posAbs="absolute|fixed".indexOf(apf.getStyle(_self.$ext,"position"))>-1;if(_self.resizable=="true"||_self.resizable=="vertical"||_self.resizable.indexOf("top")>-1||_self.resizable.indexOf("bottom")>-1){if(y<rszborder+marginBox[0]&&_self.resizable.indexOf("bottom")==-1){cursor=posAbs?"n":"";
}else{if(y>this.offsetHeight-(rszcorner||rszborder)&&_self.resizable.indexOf("top")==-1){cursor="s";
}}}if(_self.resizable=="true"||_self.resizable=="horizontal"||_self.resizable.indexOf("left")>-1||_self.resizable.indexOf("right")>-1){if(x<(cursor?rszcorner:rszborder)+marginBox[0]&&_self.resizable.indexOf("right")==-1){cursor+=tcursor+(posAbs?"w":"");
}else{if(x>this.offsetWidth-(cursor||tcursor?rszcorner:rszborder)&&_self.resizable.indexOf("left")==-1){cursor+=tcursor+"e";
...
this.style.cursor=cursor?getCssCursor(cursor)+"-resize":originalCursor||"default";
}function getCssCursor(cursor){var cssCursor=cursor;if(apf.isWebkit){if(cursor=="se"||cursor=="nw"){cssCursor="nwse";
}else{if(cursor=="sw"||cursor=="ne"){cssCursor="nesw";}else{if(cursor=="s"||cursor=="n"){cssCursor="ns";
}else{if(cursor=="e"||cursor=="w"){cssCursor="ew";}}}}}return cssCursor;}var oOutline;
};apf.GuiElement.propHandlers.resizable=function(value){this.implement(apf.Interactive);
...
apf.__MEDIA__=1<<20;apf.__MULTICHECK__=1<<22;apf.Teleport=function(){this.$init(true);
};apf.__TELEPORT__=1<<28;(function(){this.$parsePrio="002";this.$regbase=this.$regbase|apf.__TELEPORT__;
this.$booleanProperties.autoroute=true;this.$supportedProperties.push("url","timeout","protocol","route-server","autoroute");
...
apf.Init.run("teleport");apf.__TRANSACTION__=1<<3;apf.__VIRTUALVIEWPORT__=1<<19;
apf.VirtualViewport=function(){this.$init(true);this.$regbase=this.$regbase|apf.__VIRTUALVIEWPORT__;
this.virtualVTimer=null;this._xmlUpdate=this.$xmlUpdate;apf.setStyleClass(this.$ext,"virtual");
this.$deInitNode=function(xmlNode,htmlNode){this.$viewport.redraw();};this.$moveNode=function(xmlNode,htmlNode){this.$viewport.redraw();
};this.emptyNode=apf.xmldb.getXml("<empty />");this.$addEmpty=this.$add;this.$add=function(xmlNode,Lid,xmlParentNode,htmlParentNode,beforeNode){var htmlNode=this.$pHtmlDoc.getElementById(Lid);
...
}}if(!nomsg){this.$viewport.offset=0;this.$viewport.length=0;this.$viewport.resize(0);
if(this.$viewport.sb){this.$viewport.sb.$update();}if(this.$setClearMessage){this.$setClearMessage(msgType?this[msgType+"-message"]:this["empty-message"],msgType||"empty");
}}else{if(this.$removeClearMessage){this.$removeClearMessage();}}this.$viewport.cache=null;
};var _self=this;this.$viewport=new apf.$viewportVirtual(this);this.getViewport=function(){return this.$viewport;
};this.$isInViewport=function(xmlNode,struct){var nodes=this.getTraverseNodes();
...
};this.$xmlUpdate=function(){this.$viewport.cache=null;this.$viewport.length=this.xmlRoot.selectNodes(this.each).length;
this.$viewport.sb.$update(this.$container);this._xmlUpdate.apply(this,arguments);
};this.$load=function(XMLRoot){apf.xmldb.addNodeListener(XMLRoot,this);this.$updateTraverseCache(XMLRoot,true);
if(!this.renderRoot&&!this.$cachedTraverseList.length){return this.clear();}this.$viewport.cache=null;
this.$viewport.offset=0;this.$viewport.length=this.$cachedTraverseList.length;if(this.$viewport.length<this.$viewport.limit){this.$viewport.resize(this.$viewport.length);
}this.$viewport.prepare();var nodes=this.$addNodes(XMLRoot,null,null,this.renderRoot);
...
}}if(this.$focussable){apf.window.hasFocus(this)?this.$focus():this.$blur();}this.$viewport.setScrollTop(0,true);
};this.$loadSubData=function(){};this.$updateTraverseCache=function(xmlNode,force){if(force||!this.$cachedTraverseList||this.$cachedTraverseList.name!=this.each){this.$cachedTraverseList=(xmlNode||this.xmlRoot).selectNodes(this.each);
this.$cachedTraverseList.name=this.each;}};var getTraverseNodes=this.getTraverseNodes=function(xmlNode){if(!this.xmlRoot){return;
}if(this.$viewport.cache){return this.$viewport.cache;}this.$updateTraverseCache(xmlNode);
var start=this.$viewport.offset,end=Math.min(this.$cachedTraverseList.length,start+this.$viewport.limit);
var list=[];for(var i=start;i<end;i++){list.push(this.$cachedTraverseList[i]);}return this.$sort?this.$sort.apply(list):list;
};var baseNTS=this.getNextTraverseSelected;this.getNextTraverseSelected=function(xmlNode,up,count){if(!xmlNode){xmlNode=this.selected;
}if(!count){count=1;}this.getTraverseNodes=apf.MultiselectBinding.prototype.getTraverseNodes;
var node=baseNTS.call(this,xmlNode,up,count);this.getTraverseNodes=getTraverseNodes;
return node;};this.caching=false;};apf.$viewportVirtual=function(amlNode){this.amlNode=amlNode;
var _self=this;this.amlNode.addEventListener("mousescroll",function(e){if(!_self.scrollbar||_self.scrollbar.horizontal){return;
}if(e.returnValue===false){return;}var div=_self.getScrollHeight()-_self.getHeight();
if(div){_self.setScrollTop(_self.getScrollTop()+(-1*e.delta*Math.min(45,_self.getHeight()/10)),false,true);
e.preventDefault();}});apf.addListener(this.$getHtmlHost(),"scroll",function(){_self.setScrollTop(_self.getScrollTop());
});amlNode.addEventListener("resize",function(){_self.$findNewLimit();});amlNode.addEventListener("afterload",function(){var itemHeight=_self.$getItemHeight();
_self.$findNewLimit();if(itemHeight==1000){_self.$findNewLimit();}});if(!amlNode.scrollbar){this.sb=new apf.scrollbar();
this.sb.parentNode=new apf.Class().$init();this.sb.parentNode.$container=amlNode.$pHtmlNode;
this.sb.parentNode.$int=amlNode.$pHtmlNode;this.sb.dispatchEvent("DOMNodeInsertedIntoDocument");
this.sb.attach(this);}};(function(){this.offset=0;this.limit=2;this.length=0;this.host=this;
this.cache=null;this.inited=false;this.virtual=true;this.setScrollbar=function(scrollbar,onscroll){this.scrollbar=scrollbar;
this.amlNode.addEventListener("scroll",onscroll);};this.isVisible=function(){var htmlNode=this.$getHtmlHost();
return htmlNode.offsetHeight||htmlNode.offsetWidth?true:false;};this.focus=function(){if(this.amlNode.focus&&this.amlNode.$isWindowContainer!==true){this.amlNode.focus();
}};this.getScrollTop=function(){var htmlNode=this.$getHtmlHost();return(this.offset*this.$getItemHeight())+htmlNode.scrollTop;
};this.getScrollLeft=function(){var htmlNode=this.$getHtmlHost();return htmlNode.scrollLeft;
};this.getScrollHeight=function(){return(this.length)*this.$getItemHeight();};this.getScrollWidth=function(){var htmlNode=this.$getHtmlHost();
return(apf.isIE&&htmlNode.lastChild?htmlNode.lastChild.offsetLeft+htmlNode.lastChild.offsetWidth+apf.getBox(apf.getStyle(htmlNode,"padding"))[1]+(parseInt(apf.getStyle(htmlNode,"marginRight"))||0):htmlNode.scrollWidth);
};this.getHeight=function(){var htmlNode=this.$getHtmlHost();return htmlNode.tagName=="HTML"||htmlNode.tagName=="BODY"?apf.getWindowHeight():apf.getHtmlInnerHeight(htmlNode);
};this.getWidth=function(){var htmlNode=this.$getHtmlHost();return htmlNode.tagName=="HTML"||htmlNode.tagName=="BODY"?apf.getWindowHeight():apf.getHtmlInnerWidth(htmlNode);
};this.setScrollTop=function(value,preventEvent,byUser){var htmlNode=this.$getHtmlHost();
var itemHeight=this.$getItemHeight();this.change(Math.floor(value/itemHeight));
htmlNode.scrollTop=value-(this.offset*itemHeight);if(!preventEvent){this.amlNode.dispatchEvent("scroll",{direction:"vertical",byUser:byUser,viewport:this,scrollbar:this.scrollbar});
}};this.setScrollLeft=function(value,preventEvent,byUser){var htmlNode=this.$getHtmlHost();
htmlNode.scrollLeft=value;if(!preventEvent){this.amlNode.dispatchEvent("scroll",{direction:"horizontal",byUser:byUser,viewport:this,scrollbar:this.scrollbar});
}};this.scrollIntoView=function(xmlNode,toBottom){var _self=this.amlNode;var htmlNode=apf.xmldb.findHtmlNode(xmlNode,_self);
if(htmlNode&&htmlNode.offsetTop-_self.$container.scrollTop>0&&htmlNode.offsetTop+htmlNode.offsetHeight-_self.$container.scrollTop<_self.$container.offsetHeight){return;
}if(this.amlNode.$sharedScrollbarMove){this.amlNode.$sharedScrollbarMove();}var nr=apf.getChildNumber(xmlNode,apf.MultiselectBinding.prototype.getTraverseNodes.call(_self));
var itemHeight=this.$getItemHeight();this.setScrollTop(nr*itemHeight+(toBottom?itemHeight-this.getHeight():0));
};this.$getItemHeight=function(){if(this.amlNode.each&&this.amlNode.xmlRoot){var nodes=this.amlNode.getTraverseNodes();
if(nodes.length){var htmlNode=apf.xmldb.findHtmlNode(nodes[0],this.amlNode);if(htmlNode){return this.$lastItemHeight=htmlNode.offsetHeight;
}}}return this.$lastItemHeight||1000;};this.$getHtmlHost=function(){var htmlNode=this.amlNode.$int||this.amlNode.$container;
return(htmlNode.tagName=="BODY"||htmlNode.tagName=="HTML"?(apf.isSafari||apf.isChrome?document.body:htmlNode.parentNode):htmlNode);
};this.draw=function(){this.inited=true;var limit=this.limit;this.limit=0;this.resize(limit,true);
};this.redraw=function(){this.change(this.offset,this.limit,true);};this.prepare=function(){if(!this.inited){this.draw();
}var _self=this.amlNode;var nodes=_self.getTraverseNodes();if(!nodes){return;}var len=nodes.length;
var docId=apf.xmldb.getXmlDocId(_self.xmlRoot),hNodes=_self.$container.childNodes,hNode;
for(var j=0,i=0,l=hNodes.length;i<l;i++){if(hNodes[i].nodeType!=1){continue;}if(j>=len){hNodes[i].style.display="none";
}else{hNode=hNodes[i];_self.$deselect(hNode);hNode.style.display="block";apf.xmldb.nodeConnect(docId,nodes[j],hNode,_self);
}j++;}};this.resize=function(limit,updateScrollbar){this.cache=null;var _self=this.amlNode;
...
}}this.limit=limit;if(updateScrollbar&&this.sb){this.sb.$update(this.$container);
}};this.$findNewLimit=function(){if(!this.amlNode.xmlRoot||!this.amlNode.$ext.offsetHeight){return;
}var limit=Math.ceil(this.getHeight()/this.$getItemHeight()+2);if(this.amlNode.$cachedTraverseList&&this.amlNode.$cachedTraverseList.length<limit){limit=this.amlNode.$cachedTraverseList.length;
...
}}else{if(force||diff===false){len=hNodes.length;sel=_self.$getSelection();for(j=0,i=0;
i<len;i++){htmlNode=hNodes[i];if(htmlNode.nodeType==1){xmlNode=nodes[j++];apf.xmldb.nodeConnect(docId,xmlNode,htmlNode,_self);
_self.$updateNode(xmlNode,htmlNode);if(sel.indexOf(xmlNode)>-1){_self.$select(htmlNode);
}else{_self.$deselect(htmlNode);}}}}}}if(!_self.$selected&&sel&&sel.length){_self.$selected=apf.xmldb.findHtmlNode(sel[0],_self);
}var htmlNode=this.$getHtmlHost();var itemHeight=this.$getItemHeight();htmlNode.scrollTop=scrollTop-(this.offset*itemHeight);
};}).call(apf.$viewportVirtual.prototype);apf.__XFORMS__=1<<17;apf.clipboard=new apf.Class().$init();
apf.clipboard.store=null;apf.clipboard.empty=true;apf.clipboard.copied=null;apf.clipboard.put=function(item){this.store=item;
this.setProperty("data",item);this.setProperty("empty",item?false:true);};apf.clipboard.clear=function(){this.setProperty("data",null);
this.setProperty("empty",true);};apf.clipboard.get=function(){return this.store;
};apf.clipboard.$highlightSelection=function(amlNode,nodes,unset){for(var i=0,l=nodes.length;
...
}if(amlNode.hasFeature(apf.__DRAGDROP__)){var candrop=amlNode.isDropAllowed(apf.clipboard.data,selected);
if(!candrop){return false;}var action=candrop[1]&&candrop[1].action||(amlNode.$isTreeArch?"tree-append":"list-append");
amlNode.$dragDrop(selected,this.store,candrop&&candrop[1],action,null,null,null,true);
...
}}}if(!this.copied){this.$highlightSelection(amlNode,nodes,true);apf.clipboard.clear();
}amlNode.selectList(nodes);};apf.window=function(){this.$uniqueId=apf.all.push(this);
...
apf.window.destroy();});apf.addListener(document,"contextmenu",function(e){if(!e){e=event;
}var pos,ev,amlNode=apf.findHost(e.srcElement||e.target)||apf.document.activeElement||apf.document&&apf.document.documentElement;
...
}apf.dispatchEvent("mouseup",{htmlEvent:e});});var ta={INPUT:1,TEXTAREA:1,SELECT:1,EMBED:1,OBJECT:1};
...
}}}}else{apf.window.$focusDefault(amlNode,{mouse:true,ctrlKey:e.ctrlKey});}}}}}}apf.dispatchEvent("mousedown",{htmlEvent:e,amlNode:amlNode||apf.document.documentElement});
...
var eInfo={ctrlKey:e.ctrlKey,metaKey:e.metaKey,shiftKey:e.shiftKey,altKey:e.altKey,keyCode:e.keyCode,htmlEvent:e,isTextInput:isTextInput,bubbles:true};
...
}xmlParser.loadXML(message);if(!noError){this.xmlParseError(xmlParser);}}return xmlParser;
};apf.xmlParseError=function(xml){var xmlParseError=xml.parseError;if(xmlParseError!=0){throw new Error(apf.formatErrorString(1050,null,"XML Parse error on line "+xmlParseError.line,xmlParseError.reason+"Source Text:\n"+xmlParseError.srcText.replace(/\t/gi," ")));
...
if(!noError){this.xmlParseError(xmlParser);}}else{xmlParser=document.implementation.createDocument("","",null);
...
}apf.getOpacity=function(oHtml){return apf.getStyle(oHtml,"opacity");};apf.setOpacity=function(oHtml,value){oHtml.style.opacity=value;
};};apf.runWebkit=function(){if(XMLHttpRequest.prototype.sendAsBinary===undefined){if(window.ArrayBuffer){XMLHttpRequest.prototype.sendAsBinary=function(string){var bytes=Array.prototype.map.call(string,function(c){return c.charCodeAt(0)&255;
});this.send(new Uint8Array(bytes).buffer);};}}HTMLDocument.prototype.selectNodes=XMLDocument.prototype.selectNodes=function(sExpr,contextNode){if(sExpr.substr(0,2)=="//"){sExpr="."+sExpr;
...
}if("expanded|collapsed".indexOf(this.localName)==-1){apf.queue.add("reload"+node.$uniqueId,function(){node.reload();
});}if(e.currentTarget==this){(node.$bindings[this.localName]||(node.$bindings[this.localName]=[])).pushUnique(this);
...
}}else{return;}}if("expanded|collapsed".indexOf(this.localName)==-1){apf.queue.add("reload"+node.$uniqueId,function(){if(!node.$amlDestroyed){node.reload();
}});}if(node.$bindings.$isCompiled){node.$bindings.$compiled=node.$bindings.compile(this.localName!="each"&&this.localName);
...
break;default:this[prop]=value;}};this.define=function(action,func){apf.actiontracker.actions[action]=func;
...
}if(!this.$paused){UndoObj.id=this.$undostack.push(UndoObj)-1;}this.setProperty("undolength",this.$undostack.length);
...
this.setProperty("undolength",0);this.setProperty("redolength",0);this.dispatchEvent("afterchange",{action:"reset"});
...
}if(qItem){this.$execstack.unshift(qItem);}if(!this.$execstack[0].undoObj.state){this.$execstack[0].undoObj.saveChange(this.$execstack[0].undo,this);
}return;}qItem={undoObj:UndoObj.preparse(undo,this),undo:undo};this.$execstack.push(qItem)-1;
if(this.$execstack.length==1&&this.realtime){UndoObj.saveChange(undo,this);}};this.$queueNext=function(UndoObj,callback){if(!this.$execstack[0]||this.$execstack[0].undoObj!=UndoObj){throw new Error(apf.formatErrorString(0,this,"Executing Next                 action in queue","The execution stack was corrupted. This is                 a fatal error. The application should be restarted. You will                 lose all your changes. Please contact the administrator."));
...
apf.BindingColumnRule=function(struct,tagName){this.$init(tagName||"column",apf.NODE_VISIBLE,struct);
this.$className="col"+this.$uniqueId;};(function(){this.$defaultwidth="100";this.$width=0;
this.$sortable=true;this.$resizable=true;this.$movable=true;this.$cssInit=false;
this.visible=true;this.$attrExcludePropBind=apf.extend({css:1,icon:1,caption:1,eachcaption:1,eachvalue:1,each:1,icon:1},this.$attrExcludePropBind);
...
this.$booleanProperties.visible=true;this.$setParentFixedWidth=function(){var pNode=this.parentNode;
var vLeft=(pNode.$fixed)+"px";if(!this.$isFixedGrid){apf.setStyleRule("."+pNode.$baseCSSname+" .row"+pNode.$uniqueId,"paddingRight",vLeft,null,this.oWin);
apf.setStyleRule("."+pNode.$baseCSSname+" .row"+pNode.$uniqueId,"marginRight","-"+vLeft,null,pNode.oWin);
if(pNode.$container){pNode.$container.style.paddingRight=vLeft;}if(pNode.$head){pNode.$head.style.paddingRight=vLeft;
}}};this.$propHandlers.width=function(value,prop){if(!value){return;}var diff=value-this.$width;
this.$isPercentage=value&&String(value).indexOf("%")>-1;this.$width=parseFloat(value);
var pNode=this.parentNode;if(this.$isPercentage){apf.setStyleRule("."+this.$className,"width",this.$width+"%");
}else{apf.setStyleRule("."+this.$className,"width",this.$width+"px",null,pNode.oWin);
if(pNode.$amlLoaded){if(this.visible){pNode.$fixed+=diff;}this.$setParentFixedWidth();
...
};this.$propHandlers.visible=function(value,prop,el,force){var pNode=this.parentNode;
if(!force&&!this.$amlLoaded){return;}if(value){apf.setStyleRule("."+this.$className,"display","inline-block",null,this.oWin);
var size=this.$isPercentage?(this.$ext.offsetWidth-(pNode.$widthdiff-3)):this.$width;
this.resize(size,pNode,true);}else{apf.setStyleRule("."+this.$className,"display","none",null,this.oWin);
this.resize(0,pNode,true);}};this.resize=function(newsize,pNode,toggleShowHide){var hN;
if(this.$isPercentage){var oldsize=(this.visible&&this.$ext.offsetWidth?this.$ext.offsetWidth-(pNode.$widthdiff-3):0),ratio=oldsize?newsize/oldsize:1,next=[],fixed=[],total=0,node=toggleShowHide?this.$ext.parentNode.firstChild:this.$ext.nextSibling;
while(node&&node.getAttribute("hid")){hN=apf.all[node.getAttribute("hid")];if(hN.visible!==false){if(hN.$isPercentage){next.push(hN);
...
}var diffPerc,diffRatio;if(ratio==1&&total<101){ratio=total/101;diffRatio=1/ratio;
}else{if(total>101){ratio=ratio*101/total;diffRatio=ratio;}else{diffPerc=(ratio-1)*this.$width;
diffRatio=(total-diffPerc)/total;if(diffRatio<0.01&&diffRatio>0){if(newsize<20){return;
}return this.resize(newsize-10,pNode);}}}for(var n,i=0;i<next.length;i++){n=next[i];
if(n==this){continue;}n.setProperty("width",String(n.$width*diffRatio)+"%",false,true);
}if(this.visible!==false){this.setProperty("width",String(ratio*this.$width)+"%",false,true);
}}else{if(toggleShowHide){var diff=newsize;pNode.$fixed+=diff;this.$setParentFixedWidth();
}else{if(apf.isIE&&pNode.oIframe){this.$ext.style.width=newsize+"px";}this.setProperty("width",newsize,false,true);
}}};this.hide=function(){this.setProperty("visible",false,false,true);return this;
};this.show=function(){this.setProperty("visible",true,false,true);return this;
...
}if(width){this.$propHandlers.width.call(this,width);}apf.importStylesheet([["."+this.$className,"width:"+this.$width+(this.$isPercentage?"%;":"px;")+"text-align:"+this.align+";display: inline-block"],]);
...
pNode.$ext.style.cursor=t&&_self.$resizable?(apf.isWebkit?"ew-resize":"w-resize"):"default";
apf.setStyleClass(this,"",["hover","down"]);}else{pNode.$ext.style.cursor="default";
apf.setStyleClass(this,"hover",["down"]);}};if(!this.options&&pNode.options){this.$propHandlers.options.call(this,this.options=pNode.options);
}if(this.visible===false){this.$propHandlers.visible.call(this,false,null,null,true);
...
};this.$attrExcludePropBind=apf.extend({target:1,parent:1,action:1,copy:1},this.$attrExcludePropBind);
this.$propHandlers.target=this.$propHandlers.parent=this.$propHandlers.action=this.$propHandlers.copy=function(value,prop){delete this["c"+prop];
...
}s.push(rule.match,rule.value);}c[name]=apf.lm.compileMatch(s);return c;}for(name in this){if(name=="each"){continue;
...
}c[name]=apf.lm.compileMatch(s);}this.$isCompiled=true;return c;},getRuleIndex:function(name,index){var rule=this[name][index];
...
}else{this.$ext=parentNode.appendChild(document.createElement("iframe"));this.$ext.setAttribute("frameborder","0");
this.$browser=this.$ext;this.$ext.style.border="1px solid #999";this.$ext.style.background="white";
}this.$ext.className="apfbrowser";if(this.getAttribute("style")){this.$ext.setAttribute("style",this.getAttribute("style"));
}var _self=this;apf.addListener(this.$browser,"load",function(){var loc=this.contentWindow.location.href;
_self.dispatchEvent("load",{href:loc});if(loc){_self.setProperty("src",loc);}});
apf.addListener(this.$browser,"error",function(){_self.dispatchEvent("error");if(this.contentWindow.location.href){_self.setProperty("src",this.contentWindow.location.href);
...
}if(this.$lastHotkey){apf.hotkeys.remove(this.$lastHotkey[0],this.$lastHotkey[1]);
delete this.$lastHotkey[0];}if(value){this.$lastHotkey=[value];var _self=this;apf.hotkeys.register(value,this.$lastHotkey[1]=function(){_self.$setState("Over",{});
...
if(e&&e.htmlEvent){apf.stopPropagation(e.htmlEvent);}return false;}function menuOver(){var menuPressed=this.parentNode.menuIsPressed;
...
this.setValue(true);this.parentNode.menuIsPressed=this;menu.display(null,null,true,this,null,null,this.$ext.offsetWidth-2);
...
apf.aml.setElement("reset",apf.reset);apf.aml.setElement("button",apf.button);apf.checkbox=function(struct,tagName){this.$init(tagName||"checkbox",apf.NODE_VISIBLE,struct);
...
if(value==""&&this["default"]){value=this.value=apf.isTrue(this["default"]);}if(this.$values){this.checked=(typeof value!="undefined"&&value!==null&&value.toString()==this.$values[0].toString());
...
apf.colorpicker=function(struct,tagName){this.$init(tagName||"colorpicker",apf.NODE_VISIBLE,struct);
};(function(){this.value="ff0000";this.changeTimer=null;var c=apf.color;this.$supportedProperties.push("color","red","green","blue","hue","saturation","brightness","hex","skin-textbox","skin-spinner");
this.$propHandlers.red=this.$propHandlers.green=this.$propHandlers.blue=this.$propHandlers.hue=this.$propHandlers.saturation=this.$propHandlers.brightness=this.$propHandlers.hex=function(val,prop){clearTimeout(this.changeTimer);
var _self=this;this.changeTimer=$setTimeout(function(){_self.$change(prop);});};
this.$propHandlers.value=function(val){this.$restoreOriginal();};this.$restoreOriginal=function(){var hsb=c.hexToHSB(this.value);
this.hue=hsb.h;this.saturation=hsb.s;this.brightness=hsb.b;this.$change();this.oCustomColor.style.backgroundColor=(this.value.substr(0,1)!="#"?"#":"")+this.value;
};this.$change=function(prop){var hsb=c.fixHSB({h:this.hue,s:this.saturation,b:this.brightness});
var hex=c.HSBToHex(hsb);var rgb=c.HSBToRGB(hsb);this.oNewColor.style.backgroundColor="#"+hex;
if(prop!="red"&&prop!="green"&&prop!="blue"){this.setProperty("red",rgb.r);this.setProperty("green",rgb.g);
this.setProperty("blue",rgb.b);}if(prop!="hue"&&prop!="saturation"&&prop!="brightness"){this.setProperty("hue",hsb.h);
this.setProperty("saturation",hsb.s);this.setProperty("brightness",hsb.b);}if(prop!="hex"){this.setProperty("hex",hex);
}this.oSelector.style.background="#"+c.HSBToHex({h:hsb.h,s:100,b:100});this.oHue.style.top=parseInt(150-150*hsb.h/360,10)+"px";
this.oSelectorInd.style.left=parseInt(150*hsb.s/100,10)+"px";this.oSelectorInd.style.top=parseInt(150*(100-hsb.b)/100,10)+"px";
};this.$draw=function(){if(!this.id){this.setProperty("id","colorpicker"+this.$uniqueId);
...
};(function(){var treeState=this.$treeState;this.implement(apf.DataAction);this.bufferselect=false;
this.$useTable=false;this.$focussable=true;this.$isWindowContainer=-1;this.$widthdiff=0;
this.$defaultwidth=0;this.$useiframe=0;this.$needsDepth=true;this.$fixed=0;this.canrename=false;
this.$booleanProperties.iframe=true;this.$propHandlers.options=function(value){for(var i=0,l=this.$headings.length;
...
if(!rules||!rules.length){return;}this.$cssRules=[];this.$headings=rules;this.clearAllCache();
var fixed=0,found=false;for(var h,i=0,l=rules.length;i<l;i++){h=rules[i];if(h.visible!==false){if(!h.$isPercentage){fixed+=parseFloat(h.$width)||0;
...
this.$container.style.paddingRight=(vLeft-1)+"px";this.$head.style.paddingRight=(vLeft-2)+"px";
}this.$fixed=fixed;this.$first=0;this.$withContainer=e.bindings.description?true:false;
apf.importStylesheet(this.$cssRules,window);if(this.$useiframe){apf.importStylesheet(this.$cssRules,this.oWin);
}});this.$initNode=function(xmlNode,state,Lid,depth){this.$getNewContext("item");
...
oRow.setAttribute("onmouseup","if (!this.hasPassedDown) return;                var o = apf.lookup("+this.$uniqueId+");                 var xmlNode = apf.xmldb.findXmlNode(this);                 var isSelected = o.isSelected(xmlNode);                 if (o.hasFeature(apf.__DRAGDROP__))                     o.select(this, apf.getCtrlKey(event), event.shiftKey, -1);");
}else{oRow.setAttribute("onmousedown","var o = apf.lookup("+this.$uniqueId+");                var wasSelected = o.$selected == this;                o.select(this, apf.getCtrlKey(event), event.shiftKey, -1);"+(this.cellselect||this.namevalue?"o.selectCell(event, this, wasSelected);":""));
...
oc.setAttribute("ondblclick","event.cancelBubble = true");}else{cellType="cell";
this.$getNewContext(cellType);cell=this.$getLayoutNode(cellType);}}apf.setStyleClass(cell,h.$className);
if(h.css){apf.setStyleClass(cell,(apf.lm.compile(h.css))(xmlNode));}if(h.icon){var node=this.$getLayoutNode(cellType,"caption",oRow.appendChild(cell));
...
}apf.setNodeValue(this.$getLayoutNode(cellType,"caption",oRow.appendChild(cell)),h.cvalue2(xmlNode)||"&nbsp");
...
}cell.innerHTML=h.cvalue2(xmlNode)||"&nbsp";}if(h.icon){(cell.nodeType==1&&cell||cell.parentNode).style.backgroundImage="url("+apf.getAbsolutePath(this.iconPath,((h.cicon||h.$compile("icon",{nostring:true}))(xmlNode)||""))+")";
...
if(!htmlNode||htmlNode.nodeType!=1){return;}}if(this.$lastEditor&&this.$lastEditor[3]==htmlNode){return;
...
}else{this.$lastTextValue=nodes[i].nodeValue;nodes[i].nodeValue="";}}}if(ceditor.type==2){if(!this.$editors[h.$uniqueId+":"+editor]){var constr=apf.namespaces[apf.ns.aml].elements[editor];
...
if(h.tree){info.width="100% - "+(editParent.offsetLeft+parseInt(this.$getOption("treecell","editoroffset")));
}else{info.width="100%-3";}if(constr.prototype.hasFeature(apf.__MULTISELECT__)){info.caption=h.eachcaption||"[text()]";
...
}if(h["class"]){info["class"]=h["class"];}if(h.fill){info.fill=h.fill;}oEditor=this.$editors[h.$uniqueId+":"+editor]=new constr(info);
var box=apf.getBox(apf.getStyle(oEditor.$ext,"margin"));if(box[1]||box[3]){oEditor.setAttribute("width","100%+2-"+(box[1]+box[3]));
}oEditor.addEventListener("blur",function(){hideEditor.call(_self);});oEditor.parentNode=this;
oEditor.realtime=false;oEditor.$focusParent=this;oEditor.setAttribute("focussable","true");
...
};}else{oEditor=this.$editors[h.$uniqueId+":"+editor];if(oEditor.hasFeature(apf.__MULTISELECT__)&&!h.model){oEditor.setAttribute("value","[{"+this.id+".selected}::"+(v=h.value).substr(1,v.length-2)+"]");
}else{oEditor.setAttribute("value","[{"+this.id+".selected}::"+(v=h.value).substr(1,v.length-2)+"]");
}oEditor.setProperty("visible",true);editParent.appendChild(oEditor.$ext);oEditor.setAttribute("width",h.tree?"100% - "+(editParent.offsetLeft+parseInt(this.$getOption("treecell","editoroffset"))):"100%-3");
...
}else{if(!ed[0].value||ed[0].value==this.$lastTextValue){nodes[i].nodeValue=this.$lastTextValue;
}}}}this.$setStyleClass(ed[3],"",["editing"]);this.focus();}};this.addEventListener("beforeselect",hideEditor);
...
};this.showColumn=function(nr){var h=this.$headings[nr];h.show();};this.sortColumn=function(hid){var h=this.$headings[nr];
h.sort();};this.moveColumn=function(from,to){var h=this.$headings[nr];h.move(to);
};this.$draw=function(){this.$drawBase();var _self=this;this.$ext.onmousedown=function(e){_self.dispatchEvent("mousedown",{htmlEvent:e||event});
...
apf.aml.setElement("contents",apf.BindingRule);apf.divider=function(struct,tagName){this.$init(tagName||"divider",apf.NODE_VISIBLE,struct);
};(function(){this.$focussable=false;this.minwidth=0;this.minheight=0;this.implement(apf.ChildValue);
this.$childProperty="caption";this.addEventListener("AMLReparent",function(beforeNode,pNode,withinParent){if(!this.$amlLoaded){return;
...
if(apf.popup.last==this.$uniqueId){apf.popup.hide();}return false;};this.$setLabel=function(value){this.oLabel.innerHTML=value||this["initial-message"]||"";
...
var refHtml=host.$ext;document.body.appendChild(this.$ext);var x=(parseFloat(host.$getOption&&host.$getOption("main","erroffsetx")||0)),y=(parseFloat(host.$getOption&&host.$getOption("main","erroffsety")||0));
...
}};}this.$ext.onmousedown=function(e){(e||event).cancelBubble=true;};apf.popup.setContent(this.$uniqueId,this.$ext,"",null,null);
};this.$loadAml=function(x){if(!apf.isTrue(this.getAttribute("visible"))){this.hide();
...
apf.aml.setElement("errorbox",apf.errorbox);apf.filler=function(struct,tagName){this.$init(tagName||"filler",apf.NODE_VISIBLE,struct);
...
};(function(){this.minwidth=0;this.minheight=0;this.$focussable=false;this.$useLateDom=true;
this.$box=true;this.$layout=true;var MOZSTACK="-moz-stack";var input={INPUT:1,SELECT:1,TEXTAREA:1};
this.$booleanProperties.splitters=true;this.$supportedProperties.push("padding","reverse","edge","pack","align","splitters");
...
i<l;i++){if(!(node=nodes[i]).$ext||node.$ext.nodeType!=1){continue;}if(stretch&&!node[size]){node.$ext.style[size]=apf.isGecko&&(this.flex||node.flex)?(isInFixed?"1px":"auto"):(apf.isWebkit&&input[node.$ext.tagName]?"100%":(false&&apf.isWebkit&&node[this.$vbox?"minwidth":"minheight"]&&this.flex?"0px":"auto"));
...
var node,nodes=this.childNodes;for(var i=0,l=nodes.length;i<l;i++){if((node=nodes[i]).nodeFunc!=apf.NODE_VISIBLE||!node.$amlLoaded){continue;
...
}}}else{var isLast=isLastVisibleChild(this);if(!isLast){if(!this.nextSibling.$splitter&&!this.nextSibling.nosplitter&&!isFirstVisibleChild(this)&&!this.nosplitter){this.parentNode.insertBefore(this.ownerDocument.createElementNS(apf.ns.aml,"splitter"),this.nextSibling);
...
}}}}if(this.parentNode.$propHandlers.padding){this.parentNode.$propHandlers.padding.call(this.parentNode,this.parentNode.padding);
...
if(this.$amlLoaded){this.parentNode.$resize(true);}}}};function isFirstVisibleChild(amlNode){var firstChild=amlNode.parentNode.firstChild;
while(firstChild&&(firstChild.nodeFunc!=apf.NODE_VISIBLE||firstChild.visible===false||firstChild.visible==2&&apf.isFalse(firstChild.getAttribute("visible")))){firstChild=firstChild.nextSibling;
}return firstChild&&firstChild==amlNode;}function isLastVisibleChild(amlNode){var lastChild=amlNode.parentNode.lastChild;
...
}}}delete this.$noResize;if(!apf.hasFlexibleBox&&isLast){this.$resize();}}};this.unregister=function(amlNode){if(!amlNode.$propHandlers){return;
}amlNode.$propHandlers.left=amlNode.$propHandlers.top=amlNode.$propHandlers.right=amlNode.$propHandlers.bottom=null;
...
}if(e.relatedNode==this){this.unregister(e.currentTarget);}});this.addEventListener("DOMNodeInserted",function(e){if(e.currentTarget==this){if(this.visible){this.$ext.style.display=apf.CSSPREFIX2+"-box";
}return;}if(e.currentTarget.nodeType!=1||e.currentTarget.nodeFunc!=apf.NODE_VISIBLE){return;
...
this.$booleanProperties.checked=true;this.$booleanProperties.selected=true;this.$supportedProperties.push("submenu","value","match","group","icon","checked","selected","disabled","caption","type","values");
...
this.$group.$addRadio(this);};this.$propHandlers.hotkey=function(value){if(!this.$amlLoaded){var _self=this;
this.addEventListener("DOMNodeInsertedIntoDocument",function(e){if(_self.$hotkey&&_self.hotkey){apf.setNodeValue(this.$hotkey,apf.isMac?apf.hotkeys.toMacNotation(_self.hotkey):_self.hotkey);
}});}else{if(this.$hotkey){apf.setNodeValue(this.$hotkey,apf.isMac?apf.hotkeys.toMacNotation(value):value);
}}if(this.$lastHotkey){apf.hotkeys.remove(this.$lastHotkey[0],this.$lastHotkey[1]);
delete this.$lastHotkey[0];}if(value){this.$lastHotkey=[value];var _self=this;apf.hotkeys.register(value,this.$lastHotkey[1]=function(){if(_self.disabled||!_self.visible){return;
}if(_self.parentNode){var buttons=apf.document.getElementsByTagNameNS(apf.ns.aml,"button");
for(var i=0;i<buttons.length;i++){if(buttons[i].submenu==_self.parentNode.name){var btn=buttons[i];
...
}if(!this.$values){if(this.getAttribute("values")){this.$propHandlers.values.call(this,this.getAttribute("values"));
}}if(this.$values&&this.$values[value?0:1]!=this.value){this.setProperty("value",this.$values?this.$values[value?0:1]:true);
}};this.select=function(){this.parentNode.select(this.group,this.value||this.caption);
};this.check=function(){this.setProperty("value",this.$values?this.$values[0]:true);
};this.uncheck=function(){this.setProperty("value",this.$values?this.$values[1]:false);
};this.$check=function(){apf.setStyleClass(this.$ext,"selected");};this.$uncheck=function(){apf.setStyleClass(this.$ext,"",["selected"]);
};this.$propHandlers.selected=function(value){if(this.type!="radio"){return;}if(apf.isTrue(value)){if(this.$group){this.$group.setProperty("value",this.value);
}this.$check();}else{this.$uncheck();}};this.$propHandlers.disabled=function(value){if(apf.isTrue(value)||value==-1){apf.setStyleClass(this.$ext,"disabled");
...
return;}this.parentNode.$hideTree=true;if(!this.parentNode.sticky){this.parentNode.hide();
}this.parentNode.dispatchEvent("itemclick",{value:this.value||this.caption,relatedNode:this,checked:this.checked,selected:this.selected});
...
$ext.setAttribute("onclick",o+".$click()");});var _self=this;apf.addListener(this.$ext,"mouseover",function(e){if(!_self.disabled){_self.dispatchEvent("mouseover",{htmlEvent:e});
}});apf.addListener(this.$ext,"mouseout",function(e){if(!_self.disabled){_self.dispatchEvent("mouseout",{htmlEvent:e});
}});this.$caption=this.$getLayoutNode("item","caption",this.$ext);this.$icon=this.$getLayoutNode("item","icon",this.$ext);
this.$hotkey=this.$getLayoutNode("item","hotkey",this.$ext);if(!isSkinSwitch&&this.nextSibling&&this.nextSibling.$ext){this.$ext.parentNode.insertBefore(this.$ext,this.nextSibling.$ext);
...
if(!apf.loaded){var _self=this;if(apf.config.loaderAnimIndicator){_self.animateLoader(10);
}apf.addEventListener("load",function(){if(apf.isTrue(apf.config.autoHideLoading)){apf.queue.empty();
_self.hide();}else{if(apf.config.loaderAnimIndicator){_self.animateLoader(20);}}});
}});this.lastLoaderStep=0;this.animateLoader=function(step){var _self=this,loaderEl=document.getElementById(apf.config.loaderAnimIndicator);
if(!loaderEl){return;}step=apf.config.loaderWidth*(step/100);var fromStep=this.lastLoaderStep,toStep=this.lastLoaderStep+step;
this.lastLoaderStep=toStep;if(toStep>apf.config.loaderWidth){toStep=apf.config.loaderWidth;
}apf.tween.single(document.getElementById("animatedLoader"),{steps:5,anim:apf.tween.EASEOUT,type:"width",from:fromStep,to:toStep,onfinish:function(){if(toStep>=apf.config.loaderWidth){apf.queue.empty();
...
this.visible=false;this.matchhide=false;this.$booleanProperties.animate=true;this.$booleanProperties.pinned=true;
this.$booleanProperties.sticky=true;this.$booleanProperties.matchhide=true;this.$propHandlers.visible=function(value,prop,force,nofocus,hideOpener){if(value){this.$ext.style.display="block";
if(this.opener&&this.opener.localName.indexOf("item")>-1){this.opener.parentNode.$showingSubMenu=this;
}}else{this.$ext.style.display="none";var lastFocus=apf.menu.lastFocus;var opener=this.opener;
if(lastFocus!=this.opener&&this.opener&&this.opener.$blur){this.opener.$blur();
...
this.$selected=null;}this.dispatchEvent("hide",{opener:opener});}};var lastFocus;
this.display=function(x,y,noanim,opener,xmlNode,openMenuId,btnWidth){this.opener=opener;
...
}}function afterRender(){if(x===null){apf.popup.show(this.$uniqueId,{x:0,y:this.ref?0:opener.$ext.offsetHeight,animate:noanim||!this.animate?false:"fade",steps:10,ref:(this.ref||opener).$ext,allowTogether:openMenuId,autohide:!this.pinned,noleft:this.left!==undefined,setZindex:this.zindex?false:true});
}else{var bodyPos=apf.getAbsolutePosition(document.body);apf.popup.show(this.$uniqueId,{x:x-bodyPos[0],y:y-bodyPos[1]-(apf.isIE&&apf.isIE<8?1:0),animate:noanim||!this.animate?false:"fade",steps:10,allowTogether:openMenuId,autohide:!this.pinned,setZindex:this.zindex?false:true});
...
}this.xmlReference=xmlNode;this.dispatchEvent("display",{opener:opener});}this.visible=false;
if(!this.parentNode){apf.document.documentElement.appendChild(this);}if(this.$rendered!==false){this.show();
...
for(i=0;i<l;i++){if(nodes[i].group!=group){continue;}if(value&&(nodes[i].value==value||!nodes[i].value&&nodes[i].caption==value)){nodes[i].setProperty("selected",true,false,true);
}else{if(nodes[i].selected){nodes[i].setProperty("selected",false,false,true);}}}};
this.addEventListener("keydown",function(e){var node,key=e.keyCode;switch(key){case 13:if(!this.$selected){return;
}node=this.$selected;node.$down();node.$up();node.$click();break;case 27:this.hide();
break;case 38:node=this.$selected&&this.$selected.previousSibling||this.lastChild;
...
}return false;},true);function forceHide(e){if(this.$showingSubMenu||this.pinned||apf.isChildOf(e.fromElement,e.toElement)||apf.isChildOf(this,e.toElement)||(e.name!=="popuphide"&&!e.toElement)){return;
...
this.$propHandlers.modal=function(value){if(value){if(this.visible){apf.plane.show(this.$ext,false,null,null,{color:"black",opacity:this.cover&&this.cover.getAttribute("opacity")||0.5,protect:this.$uniqueId,customCover:this.cover||""});
...
}this.$ext.style.left=(Math.max(0,((size[0]-parseInt((this.width||this.$ext.offsetWidth)||0))/2))+size[2])+"px";
...
}if(this.modal){apf.plane.show(this.$ext,false,null,null,{color:"black",opacity:this.cover&&this.cover.getAttribute("opacity")||0.5,protect:this.$uniqueId,customCover:this.cover||""});
...
this.cover=this.$getLayoutNode("cover");if(this.popout){this.$ext.style.position="absolute";
}if(this.oIcon){this.oIcon.style.display="none";}var _self=this;if(this.oDrag){this.oDrag.host=this;
this.oDrag.onmousedown=function(e){if(!e){e=event;}if(!_self.$isWidget&&(!_self.aData||!_self.dockable||_self.aData.hidden==3)){apf.WinServer.setTop(_self);
...
};(function(){this.$parsePrio="020";this.$isModel=true;this.$createModel=true;this.canHaveChildren=false;
...
this.$booleanProperties.whitespace=true;this.$booleanProperties["create-model"]=true;
this.$booleanProperties.autoinit=true;this.$booleanProperties.enablereset=true;
this.$supportedProperties=["submission","src","session","autoinit","enablereset","remote","whitespace","create-model"];
this.$propHandlers.src=this.$propHandlers.get=function(value,prop){if(this.$amlLoaded){this.$loadFrom(value);
}};this.$propHandlers["create-model"]=function(value,prop){this.$createModel=value;
...
if(amlNode.xmlRoot!=xmlNode){amlNode.load(xmlNode);}}else{this.$waitForXml(amlNode);
}};this.$loadInAmlProp=function(id,xmlNode){var prop,node,p=this.$propBinds[id],amlNode=apf.all[id];
if(!amlNode){delete this.$propBinds[id];return;}for(prop in p){if(xmlNode&&(node=p[prop].listen?xmlNode.selectSingleNode(p[prop].listen):xmlNode)){apf.xmldb.addNodeListener(xmlNode,amlNode,"p|"+id+"|"+prop+"|"+this.$uniqueId);
...
}}if(listenRoot){p.listen=".";}if(this.data){var xmlNode=(p.listen?this.data.selectSingleNode(p.listen):this.data);
...
}else{return this.$loadFrom(xmlNode,options);}}if(this.ownerDocument&&this.ownerDocument.$domParser.$isPaused(this)){var _self=this;
this.data=xmlNode;apf.xmldb.getXmlDocId(xmlNode,this);this.$queueLoading=true;apf.queue.add("modelload"+this.$uniqueId,function(){if(_self.ownerDocument&&_self.ownerDocument.$domParser.$isPaused(_self)){apf.queue.add("modelload"+_self.$uniqueId,arguments.callee);
...
}xmlNode=apf.getXmlDom(xmlNode).documentElement;}else{return this.$insertFrom(xmlNode,options);
...
page.parentNode.remove(page,e);},false);btncontainer.appendChild(elBtnClose);}}};
this.$propHandlers.tooltip=function(value){if(!this.parentNode){return;}var node=this.parentNode.$getLayoutNode("button","caption",this.$button);
(node.nodeType==1?node:node.parentNode).setAttribute("title",value||"");};this.$propHandlers.caption=function(value){if(!this.parentNode){return;
}var node=this.parentNode.$getLayoutNode("button","caption",this.$button);if(node.nodeType==1){node.innerHTML=value;
}else{node.nodeValue=value;}};this.$propHandlers.icon=function(value){if(!this.parentNode){return;
...
}};function $btnSet(oHtml){this.parentNode.set(this);this.canHaveChildren=2;this.$setStyleClass(oHtml,"down",null,true);
...
oHtml.style.position="absolute";var div=document.createElement("div");div.style.width=oHtml.offsetWidth+"px";
div.style.marginLeft=apf.getStyle(this.$button,"marginLeft");div.style.marginRight=apf.getStyle(this.$button,"marginRight");
this.$button.parentNode.insertBefore(div,this.$button);var marginWidth=Math.abs(apf.getMargin(div)[0]);
var mUp,mMove,_self=this,started;apf.addListener(document,"mousemove",mMove=function(e){if(!e){e=event;
}if(!started){if(Math.abs(start-e.clientX)<3){return;}started=true;oHtml.style.zIndex=1;
}oHtml.style.left="-2000px";var el=document.elementFromPoint(e.clientX,t+1);var aml=el&&apf.findHost(el);
oHtml.style.left=(e.clientX-x)+"px";if(aml&&aml.localName=="page"){aml.$button.style.position="relative";
var obj,onRight=div.offsetLeft>aml.$button.offsetLeft;var pos=apf.getAbsolutePosition(aml.$button);
if(onRight&&aml.$button.offsetWidth-e.clientX+pos[0]<marginWidth){return;}if(obj=_self.$btnControl[aml.$uniqueId]){if(obj.onRight!=onRight){obj.stop();
}else{return;}}_self.$btnControl[aml.$uniqueId]={onRight:onRight};var newPosition=_self.$lastPosition=onRight?aml.$button:aml.$button.nextSibling;
_self.$lastLeft=aml.$button.offsetLeft;apf.tween.single(aml.$button,{steps:20,interval:10,from:0,to:onRight?aml.$button.offsetWidth-marginWidth:-1*(aml.$button.offsetWidth-marginWidth),type:"left",anim:apf.tween.easeInOutCubic,control:_self.$btnControl[aml.$uniqueId],onstop:function(){},onfinish:function(){aml.$button.style.left=aml.$button.style.position="";
delete _self.$btnControl[aml.$uniqueId];if(div&&div.parentNode){div.parentNode.insertBefore(div,newPosition);
...
this.addEventListener("prop.class",function(e){apf.setStyleClass(this.$button,e.value,this.$lastClassValueBtn?[this.$lastClassValueBtn]:null);
...
this.$button=apf.insertHtmlNode(elBtn,this.parentNode.$buttons);var closebtn=this.closebtn=this.getAttribute("closebtn");
if((apf.isTrue(closebtn)||((this.parentNode.buttons||"").indexOf("close")>-1&&!apf.isFalse(closebtn)))){this.$propHandlers.closebtn.call(this,true);
...
if(this.$isLast){this.$last();}if(this.$isFirst){this.$first();}};this.$destroy=function(){if(this.$button){if(this.parentNode&&!this.parentNode.$amlDestroyed&&this.$button.parentNode){this.$button.parentNode.removeChild(this.$button);
}this.$button.host=null;this.$button=null;}};}).call(apf.page.prototype=new apf.Presentation());
apf.aml.setElement("page",apf.page);apf.param=function(struct,tagName){this.$init(tagName||"param",apf.NODE_HIDDEN,struct);
...
this.$booleanProperties.selected=true;this.$supportedProperties.push("value","background","group","label","selected","tooltip","icon");
this.$propHandlers.group=function(value){if(!this.$ext){return;}if(this.$group&&this.$group.$removeRadio){this.$group.$removeRadio(this);
}if(!value){this.$group=null;return;}var group=typeof value=="string"?apf.nameserver.get("group",value):value;
if(!group){group=apf.nameserver.register("group",value,new apf.$group());group.setAttribute("id",value);
group.dispatchEvent("DOMNodeInsertedIntoDocument");group.parentNode=this;}this.$group=group;
...
}};this.$propHandlers.selected=function(value){if(!this.$group){return;}if(value){this.$group.setProperty("value",this.value);
...
};this.select=function(){this.setProperty("selected",true,false,true);};this.getGroup=function(){return this.$group;
};this.$check=function(visually){this.$setStyleClass(this.$ext,this.$baseCSSname+"Selected");
this.selected=true;if(this.oInput){this.oInput.selected=true;}this.doBgSwitch(2);
};this.$uncheck=function(){this.$setStyleClass(this.$ext,"",[this.$baseCSSname+"Selected"]);
this.selected=false;if(this.oInput){this.oInput.selected=false;}this.doBgSwitch(1);
...
apf.aml.setElement("radiobutton",apf.radiobutton);apf.$group=apf.group=function(struct,tagName){this.$init(tagName||"group",apf.NODE_VISIBLE,struct);
...
if(!rb.value){rb.setProperty("value",id);}var _self=this;rb.addEventListener("prop.value",function(e){if(this.selected){_self.setProperty("value",e.value);
}else{if(_self.value==e.value){this.select();}}});if(this.value&&rb.value==this.value){this.setProperty("selectedItem",rb);
}else{if(rb.selected){this.setProperty("value",rb.value);}}};this.$removeRadio=function(rb){radiobuttons.remove(rb);
...
};(function(){this.realtime=true;this.visible=false;this.overflow="scroll";this.position=0;
this.$visible=true;this.$scrollSizeValue=0;this.$stepValue=0.03;this.$bigStepValue=0.1;
this.$timer=null;this.$scrollSizeWait;this.$slideMaxSize;this.$booleanProperties=["showonscroll"];
this.addEventListener("focus",function(){this.$viewport.focus();});this.$propHandlers.showonscroll=function(value){clearTimeout(this.$hideOnScrollTimer);
if(value){this.$ext.style.display="none";}else{this.$ext.style.display="block";
this.show();}};this.$propHandlers.overflow=function(value){if(this.showonscroll){return;
}if(value=="auto"){this.$ext.style.display="none";this.$resize();}else{if(value=="scroll"){this.setProperty("visible",true);
}}};this.$propHandlers["for"]=function(value){if(value){var amlNode=typeof value=="string"?self[value]:value;
...
});}});}this.$recalc();this.$update();return this;};this.$resize=function(){if(!this.$viewport||!this.$viewport.isVisible()||!this.$viewport.virtual){return;
...
};this.$update=function(){if(this.showonscroll&&!this.$ext.offsetHeight){return;
}var viewport=this.$viewport;if(!viewport||!viewport.isVisible()){return;}this.$updating=true;
var vp=viewport.getHeight();var sz=viewport.getScrollHeight();if(vp>=sz){if(this.overflow=="scroll"){this.$caret.style.display="none";
this.disable();}else{if(this.visible){this.hide();}}}else{if(this.overflow=="scroll"){this.$caret.style.display="block";
...
this.position=viewport.getScrollTop()/(sz-vp);var bUpHeight=this.$btnUp?this.$btnUp[this.$offsetSize]:0;
this.$caret.style[this.$pos]=(bUpHeight+(apf[this.$getInner](this.$caret.parentNode)-(bUpHeight*2)-this.$caret[this.$offsetSize])*this.position)+"px";
}this.$updating=false;};this.setScrollPosition=function(position,preventEvent){if(position==NaN){return;
}if(position>1){position=1;}if(position<0){position=0;}this.position=position;var bUpHeight=this.$btnUp?this.$btnUp[this.$offsetSize]:0;
this.$caret.style[this.$pos]=(bUpHeight+(apf[this.$getInner](this.$caret.parentNode)-(bUpHeight*2)-this.$caret[this.$offsetSize])*this.position)+"px";
if(this.animating||!this.$visible){return;}var vp=this.$viewport;var to=(vp.getScrollHeight()-vp.getHeight())*position;
vp.setScrollTop(to,preventEvent);};this.animShowScrollbar=function(timeout,cb){var _self=this;
return setTimeout(function(){_self.$ext.style.display="block";if(_self.$showOnScrollControl&&_self.$showOnScrollControl.state==apf.tween.RUNNING){return;
}if(_self.$hideOnScrollControl){_self.$hideOnScrollControl.stop();}apf.tween.single(_self.$ext,{control:_self.$hideOnScrollControl={},type:"fade",from:0,to:1,onfinish:function(){cb&&cb();
}});},timeout);};this.animHideScrollbar=function(timeout,cb){var _self=this;return setTimeout(function(){if(_self.$hideOnScrollControl&&_self.$hideOnScrollControl.state==apf.tween.RUNNING){return;
}if(_self.$showOnScrollControl){_self.$showOnScrollControl.stop();}apf.tween.single(_self.$ext,{control:_self.$hideOnScrollControl={},type:"fade",from:1,to:0,steps:20,onfinish:function(){_self.$ext.style.display="none";
apf.setOpacity(_self.$ext,1);cb&&cb();}});},timeout);};this.scrollUp=function(v){if(v>this.$caret[this.$offsetPos]){return this.$ext.onmouseup();
}this.setScrollPosition(this.position+this.$bigStepValue);if(this.$slideFast){this.$slideFast.style[this.$size]=Math.max(1,this.$caret[this.$offsetPos]-this.$btnUp[this.$offsetSize])+"px";
...
}this.setScrollPosition(this.position+this.$bigStepValue);if(this.$slideFast){this.$slideFast.style[this.$pos]=(this.$caret[this.$offsetPos]+this.$caret[this.$offsetSize])+"px";
...
}this.className="btnup btnupdown";clearTimeout(_self.$timer);_self.setScrollPosition(_self.position-_self.$stepValue);
apf.stopPropagation(e);_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.setScrollPosition(_self.position-_self.$stepValue);
},20);},300);};this.$btnUp.onmouseout=this.$btnUp.onmouseup=function(){if(_self.disabled){return;
...
_self.setScrollPosition(_self.position+_self.$stepValue);apf.stopPropagation(e);
_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.setScrollPosition(_self.position+_self.$stepValue);
},20);},300);};this.$btnDown.onmouseout=this.$btnDown.onmouseup=function(){if(_self.disabled){return;
...
_self.dispatchEvent("mousedown",{});_self.dragging=true;document.onmousemove=function(e){if(!e){e=event;
...
if(next>max){next=max;}_self.setScrollPosition((next-min)/(max-min));};document.onmouseup=function(){_self.$startPos=false;
if(!_self.realtime){_self.setScrollPosition(_self.position);}if(this.releaseCapture){this.releaseCapture();
}_self.$setStyleClass(_self.$ext,"",[_self.$baseCSSname+"Down"]);_self.dispatchEvent("mouseup",{});
_self.dragging=false;document.onmouseup=document.onmousemove=null;};apf.stopPropagation(e);
return false;};this.$ext.onmousedown=function(e){if(_self.disabled){return;}if(!e){e=event;
}clearInterval(_self.$timer);var offset;if(e[_self.$eventDir]>_self.$caret[_self.$offsetPos]+_self.$caret[_self.$offsetSize]){_self.setScrollPosition(_self.position+_self.$bigStepValue);
if(_self.$slideFast){_self.$slideFast.style.display="block";_self.$slideFast.style[_self.$pos]=(_self.$caret[_self.$offsetPos]+_self.$caret[_self.$offsetSize])+"px";
...
}offset=e[_self.$eventDir];_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.scrollDown(offset,null,null,true);
},20);},300);}else{if(e[_self.$eventDir]<_self.$caret[_self.$offsetPos]){_self.setScrollPosition(_self.position-_self.$bigStepValue);
if(_self.$slideFast){_self.$slideFast.style.display="block";_self.$slideFast.style[_self.$pos]=_self.$btnUp[_self.$offsetSize]+"px";
_self.$slideFast.style[_self.$size]=(_self.$caret[_self.$offsetPos]-_self.$btnUp[_self.$offsetSize])+"px";
}offset=e[_self.$eventDir];_self.$timer=$setTimeout(function(){_self.$timer=setInterval(function(){_self.scrollUp(offset,null,null,true);
},20);},300);}}};this.$ext.onmouseup=function(){if(_self.disabled){return;}clearInterval(_self.$timer);
if(!_self.realtime){_self.setScrollPosition(_self.position);}if(_self.$slideFast){_self.$slideFast.style.display="none";
}};this.$ext.onmouseover=function(e){_self.dispatchEvent("mouseover",{htmlEvent:e||event});
};this.$ext.onmouseout=function(e){_self.dispatchEvent("mouseout",{htmlEvent:e||event});
...
apf.aml.setElement("scrollbar",apf.scrollbar);apf.GuiElement.propHandlers.scrollbar=function(value){if(this.$sharedScrollbar==undefined){var values=value.split(" ");
var name=values[0];var top=values[1]||0;var right=values[2]||0;var bottom=values[3]||0;
var _self=this;this.$sharedScrollbar=self[name]||false;function hasOnScroll(){return sb&&apf.isTrue(sb.getAttribute("showonscroll"));
...
}sb=_self.$sharedScrollbar;if(!sb.$addedMouseOut){apf.addListener(sb.$ext,"mouseout",function(e){if(!hasOnScroll()){return;
}if(apf.findHost(e.fromElement)==sb&&apf.findHost(e.toElement)!=sb){clearTimeout(timer);
hideScrollbar();}});sb.$addedMouseOut=true;}if(!sb.$viewport||sb.$viewport.amlNode!=_self){var pNode=(_self.$ext==oHtml?_self.$ext.parentNode:_self.$ext);
...
}}if(hasOnScroll()&&e){clearTimeout(timer);var pos=apf.getAbsolutePosition(oHtml);
var rightPos=oHtml.offsetWidth-(e.clientX-pos[0]);var show=rightPos<25&&rightPos>right;
if(show&&sb.$ext.style.display=="none"||!show&&sb.$ext.style.display=="block"){if(show){showScrollbar();
}else{hideScrollbar();}}else{if(!show){sb.showonscroll=true;}}}});this.$sharedScrollbarMove=mouseMove;
function showScrollbar(){sb.setProperty("showonscroll",false);sb.$ext.style.display="none";
timer=sb.animShowScrollbar(200);}function hideScrollbar(timeout){if(sb.scrolling){return;
}if(!sb.dragging){timer=sb.animHideScrollbar(timeout||200,function(){sb.setProperty("showonscroll",true);
});}else{apf.addListener(document,"mouseup",function(e){var tgt=apf.findHost(e.target);
if(tgt==sb){return;}if(tgt==_self){mouseMove(e);}else{hideScrollbar();}apf.removeListener(document,"mouseup",arguments.callee);
});}}apf.addListener(oHtml,"mouseout",function(e){if(!hasOnScroll()){return;}if(apf.findHost(e.toElement)!=sb){clearTimeout(timer);
hideScrollbar();}});}};apf.ViewPortAml=function(amlNode){this.amlNode=amlNode;var _self=this;
var update=function(){if(_self.scrollbar){_self.scrollbar.$update();}};amlNode.addEventListener("resize",update);
if(amlNode.hasFeature(apf.__DATABINDING__)){amlNode.addEventListener("afterload",update);
amlNode.addEventListener("xmlupdate",update);}amlNode.addEventListener("prop.value",update);
if(amlNode.$isTreeArch){amlNode.addEventListener("collapse",update);amlNode.addEventListener("expand",update);
}amlNode.addEventListener("mousescroll",function(e){_self.$mousescroll(e);});var htmlNode=_self.$getHtmlHost();
apf.addListener(htmlNode,"scroll",function(){if(_self.scrollbar.animating||!_self.scrollbar.$visible){return;
}_self.setScrollTop(this.scrollTop);});if("HTML|BODY".indexOf(htmlNode.tagName)>-1){var lastHeight=htmlNode.scrollHeight;
setInterval(function(){if(lastHeight!=htmlNode.scrollHeight){lastHeight=htmlNode.scrollHeight;
_self.scrollbar.$recalc();_self.scrollbar.$update();}},100);}};(function(){this.setScrollbar=function(scrollbar,onscroll){this.scrollbar=scrollbar;
this.amlNode.addEventListener("scroll",onscroll);};this.isVisible=function(){var htmlNode=this.$getHtmlHost();
return htmlNode.offsetHeight||htmlNode.offsetWidth?true:false;};this.focus=function(){if(this.amlNode.focus&&this.amlNode.$isWindowContainer!==true){this.amlNode.focus();
}};this.getScrollTop=function(){var htmlNode=this.$getHtmlHost();return htmlNode.scrollTop;
};this.getScrollLeft=function(){var htmlNode=this.$getHtmlHost();return htmlNode.scrollLeft;
};this.getScrollHeight=function(){var htmlNode=this.$getHtmlHost();return(apf.isIE&&htmlNode.lastChild?htmlNode.lastChild.offsetTop+htmlNode.lastChild.offsetHeight+apf.getBox(apf.getStyle(htmlNode,"padding"))[2]+(parseInt(apf.getStyle(htmlNode,"marginBottom"))||0):htmlNode.scrollHeight);
};this.getScrollWidth=function(){var htmlNode=this.$getHtmlHost();return(apf.isIE&&htmlNode.lastChild?htmlNode.lastChild.offsetLeft+htmlNode.lastChild.offsetWidth+apf.getBox(apf.getStyle(htmlNode,"padding"))[1]+(parseInt(apf.getStyle(htmlNode,"marginRight"))||0):htmlNode.scrollWidth);
};this.getHeight=function(){var htmlNode=this.$getHtmlHost();return htmlNode.tagName=="HTML"||htmlNode.tagName=="BODY"?apf.getWindowHeight():apf.getHtmlInnerHeight(htmlNode);
};this.getWidth=function(){var htmlNode=this.$getHtmlHost();return htmlNode.tagName=="HTML"||htmlNode.tagName=="BODY"?apf.getWindowHeight():apf.getHtmlInnerWidth(htmlNode);
};this.setScrollTop=function(value,preventEvent,byUser){var htmlNode=this.$getHtmlHost();
htmlNode.scrollTop=value;if(!preventEvent){this.amlNode.dispatchEvent("scroll",{direction:"vertical",byUser:byUser,viewport:this,scrollbar:this.scrollbar});
}};this.setScrollLeft=function(value,preventEvent,byUser){var htmlNode=this.$getHtmlHost();
htmlNode.scrollLeft=value;if(!preventEvent){this.amlNode.dispatchEvent("scroll",{direction:"horizontal",byUser:byUser,viewport:this,scrollbar:this.scrollbar});
}};this.$getHtmlHost=function(){var htmlNode=this.amlNode.$int||this.amlNode.$container;
return(htmlNode.tagName=="BODY"||htmlNode.tagName=="HTML"?(apf.isSafari||apf.isChrome?document.body:htmlNode.parentNode):htmlNode);
};this.$mousescroll=function(e){if(this.scrollbar.horizontal){return;}if(e.returnValue===false){return;
}var oHtml=this.$getHtmlHost();var sb=this.scrollbar;var div=this.getScrollHeight()-this.getHeight();
if(div){if(oHtml[sb.$scrollPos]==0&&e.delta>0){if(this.$lastScrollState===0){return;
}setTimeout(function(){this.$lastScrollState=0;},300);}else{if(oHtml[sb.$scrollPos]==this.getScrollHeight()-oHtml[sb.$offsetSize]&&e.delta<0){if(this.$lastScrollState===1){return;
}setTimeout(function(){this.$lastScrollState=1;},300);}}delete this.$lastScrollState;
this.setScrollTop(this.getScrollTop()+-1*e.delta*Math.min(45,this.getHeight()/10),false,true);
e.preventDefault();}};}).call(apf.ViewPortAml.prototype);apf.ViewPortHtml=function(htmlNode){this.$getHtmlHost=function(){return htmlNode;
};var _self=this;htmlNode=(htmlNode.tagName=="BODY"||htmlNode.tagName=="HTML"?(apf.isSafari||apf.isChrome?document.body:htmlNode.parentNode):htmlNode);
apf.addEventListener("mousescroll",function(e){if(htmlNode==e.target||(htmlNode==document.documentElement&&e.target==document.body)){_self.$mousescroll(e);
}});apf.addListener(htmlNode,"scroll",function(){_self.setScrollTop(this.scrollTop);
});if("HTML|BODY".indexOf(htmlNode.tagName)>-1){var lastHeight=htmlNode.scrollHeight;
setInterval(function(){if(lastHeight!=htmlNode.scrollHeight){lastHeight=htmlNode.scrollHeight;
_self.scrollbar.$recalc();_self.scrollbar.$update();}},100);}this.amlNode=new apf.Class().$init();
};apf.ViewPortHtml.prototype=apf.ViewPortAml.prototype;apf.skin=function(struct,tagName){this.$init(tagName||"skin",apf.NODE_HIDDEN,struct);
...
}var defer=this.attributes.getNamedItem("defer");if(!defer||!apf.isTrue(defer.nodeValue)){domParser.$pauseParsing.apply(domParser,this.$parseContext=domParser.$parseContext||[this.ownerDocument.documentElement]);
}loadSkinFile.call(this,path);}function finish(xmlNode){if(xmlNode){apf.skins.Init(xmlNode,this,this.$path);
}if(!this.defer){var domParser=this.ownerDocument.$domParser;domParser.$continueParsing(this.$parseContext[0]);
...
if(extra.tpModule.retryTimeout(extra,state,null,oError)===true){return true;}throw oError;
}xmlString=xmlString.replace(/\<\!DOCTYPE[^>]*>/,"").replace(/^[\r\n\s]*/,"").replace(/xmlns\=\"[^"]*\"/g,"");
...
}this.addEventListener("DOMNodeInsertedIntoDocument",function(e){if(this.src||this.name){return;
...
}this.focused=true;this.$setStyleClass(this.oInput,"focus");this.$setStyleClass(this.$buttonPlus,"plusFocus");
this.$setStyleClass(this.$buttonMinus,"minusFocus");if(this.oLeft){this.$setStyleClass(this.oLeft,"leftFocus");
}};this.$blur=function(e){if(!this.$ext&&!this.focused){return;}this.$setStyleClass(this.oInput,"",["focus"]);
this.$setStyleClass(this.$buttonPlus,"",["plusFocus"]);this.$setStyleClass(this.$buttonMinus,"",["minusFocus"]);
if(this.oLeft){this.$setStyleClass(this.oLeft,""["leftFocus"]);}this.setValue(this.oInput.value);
this.focused=false;};this.addEventListener("keydown",function(e){var key=e.keyCode,keyAccess=(key<8||(key>9&&key<37&&key!==12)||(key>40&&key<46)||(key>46&&key<48)||(key>57&&key<96)||(key>105&&key<109&&key!==107)||(key>109&&key!==189));
...
this.oLeft=this.$getLayoutNode("main","left",this.$ext);var timer,doc=(!document.compatMode||document.compatMode=="CSS1Compat")?document.html:document.body,z=0;
...
};this.$propHandlers.tooltip=function(value){this.$button1.setProperty("tooltip",value);
};this.$propHandlers.hotkey=function(value){this.$button1.setProperty("hotkey",value);
...
};(function(){this.minwidth=0;this.minheight=0;this.$scale=0;this.$focussable=false;
this.$splitter=true;this.$booleanProperties.realtime=true;this.$propHandlers.realtime=function(value){this.$setStyleClass(this.$ext,value&&(this.$baseCSSname+"Realtime")||"",[this.$baseCSSname+"Realtime"]);
};this.$propHandlers.scale=function(value){this.$scale=value=="left"||value=="top"?1:(value=="right"||"bottom "?2:0);
};this.$propHandlers.parent=function(value){this.$parent=typeof value=="object"?value:self[value];
...
}this.init();});this.update=function(newPos,finalPass){with(this.$info){var max=this.$previous?this.$previous.$ext[offsetSize]+this.$next.$ext[offsetSize]:(this.parentNode).getWidth();
var method=finalPass?"setAttribute":"setProperty";if(apf.hasFlexibleBox){newPos-=this.$previous?apf.getAbsolutePosition(this.$previous.$ext,this.parentNode.$int)[d1]:0;
}if(this.$previous&&this.$next&&(this.$previous.flex||this.$previous.flex===0)&&(this.$next.flex||this.$next.flex===0)){if(!finalPass&&!this.realtime){newPos-=this.$ext[offsetSize];
...
}}else{if(this.$next&&!this.$next.flex&&(!this.$scale||this.$scale==2)){this.$next[method](osize,max-newPos);
}if(this.$previous&&!this.$previous.flex&&(!this.$scale||this.$scale==1)){this.$previous[method](osize,newPos);
...
if(this.parentNode&&this.parentNode.$box){this.setProperty("type",this.parentNode.localName=="vbox"?"horizontal":"vertical");
...
var _self=this;this.$ext.onmousedown=function(e){if(!e){e=event;}apf.dragMode=true;
_self.$setSiblings();var changedPosition,pHtml=_self.parentNode.$int,diff=0;if("absolute|fixed|relative".indexOf(apf.getStyle(pHtml,"position"))==-1){pHtml.style.position="relative";
changedPosition=true;}_self.$totalFlex=0;with(_self.$info){if(_self.$parent){if(!_self.$previous){var posNext=apf.getAbsolutePosition(_self.$next.$ext,_self.parentNode.$int);
var wd=_self.$parent.getWidth();if(_self.$scale==2){var max=posNext[d1]+_self.$next.$ext[offsetSize]-this[offsetSize];
diff=(_self.parentNode.$int[offsetSize]-max);var min=max-wd-diff;}}else{if(!_self.$next){}}}else{if(_self.$previous){var posPrev=apf.getAbsolutePosition(_self.$previous.$ext,_self.parentNode.$int);
var min=_self.$scale?0:posPrev[d1]||0;}if(_self.$next){var posNext=apf.getAbsolutePosition(_self.$next.$ext,_self.parentNode.$int);
var max=posNext[d1]+_self.$next.$ext[offsetSize]-this[offsetSize];}}if(_self.$previous&&_self.$next){if((_self.$previous.flex||_self.$previous.flex===0)&&(_self.$next.flex||_self.$next.flex===0)){var set=[],nodes=_self.parentNode.childNodes,padding=0;
...
startPos=e[clientPos]-coords[d1];if(!_self.realtime){if(_self.$previous.flex&&!_self.$next.flex){var mBox=apf.getBox(_self.$next.margin);
...
this.style.position="relative";}min=-1000;}}apf.plane.show(this);_self.$setStyleClass(this,_self.$baseCSSname+"Moving");
_self.$setStyleClass(document.body,_self.type=="vertical"?"w-resize":"n-resize",[_self.type=="vertical"?"n-resize":"w-resize"]);
_self.dispatchEvent("dragstart");document.onmouseup=function(e){if(!e){e=event;
}with(_self.$info){var newPos;if(e[clientPos]>=0){var coords=apf.getAbsolutePosition(_self.$ext.offsetParent);
newPos=(Math.min(max,Math.max(min,(e[clientPos]-coords[d1])-(apf.hasFlexibleBox?startPos:startOffset))))+diff;
...
}_self.dispatchEvent("dragdrop");document.onmouseup=document.onmousemove=null;apf.dragMode=false;
};document.onmousemove=function(e){if(!e){e=event;}with(_self.$info){var newPos;
if(e[clientPos]>=0){var coords=apf.getAbsolutePosition(_self.$ext.offsetParent);
newPos=(Math.min(max,Math.max(min,(e[clientPos]-coords[d1])-(apf.hasFlexibleBox||!_self.realtime?startPos:startOffset))))+diff;
if(_self.realtime){_self.update(newPos);}else{_self.$ext.style[pos]=newPos+"px";
}}}apf.stopEvent(e);_self.dispatchEvent("dragmove");};};apf.queue.add("splitter"+this.$uniqueId,function(){_self.init();
...
apf["switch"]=function(struct,tagName){this.$hasButtons=false;this.$init(tagName||"switch",apf.NODE_VISIBLE,struct);
};apf.pages=function(struct,tagName){this.$hasButtons=false;this.$init(tagName||"pages",apf.NODE_VISIBLE,struct);
this.$focussable=false;};apf.tab=function(struct,tagName){this.$hasButtons=true;
...
apf.addListener(this.$scrollArea,"scroll",this.$scrollFunc=function(){_self.$scrolldown=this.scrollTop>=this.scrollHeight-this.offsetHeight+apf.getVerBorders(this);
...
}this.oDrag=this.oIframe=this.oFocus=this.$container=this.$ext=null;});}).call(apf.text.prototype=new apf.StandardBinding());
...
};this.$propHandlers["initial-message"]=function(value){if(value){}if(!this.value){this.$clear(true);
}if(this.type=="password"&&this.$inputInitFix){this.$inputInitFix.innerHTML=value;
apf.setStyleClass(this.$inputInitFix,"initFxEnabled");}};this.$propHandlers.rows=function(value){if(this.$input.tagName.toLowerCase()=="textarea"&&value){this.setProperty("rows",value);
if(this.$ext){this.$ext.rows=value;}}};this.$propHandlers.cols=function(value){if(this.$input.tagName.toLowerCase()=="textarea"&&value){this.setProperty("cols",value);
if(this.$ext){this.$ext.cols=value;}}};this.$propHandlers.focusselect=function(value){var _self=this;
...
}};if(apf.hasAutocompleteXulBug){this.$input.setAttribute("autocomplete","off");
...
this.$initNode=function(xmlNode,state,Lid){this.$getNewContext("item");var hasChildren=state&HAS_CHILD||this.emptyMessage&&this.$applyBindRule("empty",xmlNode),oItem=this.$getLayoutNode("item");
...
elOpenClose.setAttribute("ondblclick","event.cancelBubble = true");}var ocAction=this.opencloseaction||"ondblclick";
var elIcon=this.$getLayoutNode("item","icon");if(elIcon&&elIcon!=elOpenClose){if(ocAction!="ondblclick"){elIcon.setAttribute(ocAction,"var o = apf.lookup("+this.$uniqueId+");"+(ocAction=="onmousedown"?"o.select(this, apf.getCtrlKey(event), event.shiftKey, event.button);":"")+(true?"o.slideToggle(this, null, null, true);":""));
}if(ocAction!="onmousedown"){elIcon.setAttribute("onmousedown","apf.lookup("+this.$uniqueId+").select(this, apf.getCtrlKey(event), event.shiftKey, event.button);");
...
}}}var elCaption=this.$getLayoutNode("item","caption");if(elCaption){apf.setNodeValue(elCaption,this.$applyBindRule("caption",xmlNode));
...
this.$supportedProperties.push("showhidden","force-host");this.$propHandlers.showhidden=function(value){this.$showHidden=value;
...
}var fHost=this["force-host"];if(fHost&&sPath.indexOf(fHost)===-1){sPath=fHost.replace(/[\/]+$/,"")+"/"+sPath.replace(/^[\/]+/,"");
...
}else{throw oError;}}extra.headers=oHeaders;var iStatus=parseInt(extra.status,10);
...
});};this.readFile=this.read=function(sPath,callback){this.method="GET";this.doRequest(function(data,state,extra){var iStatus=parseInt(extra.status,10);
...
var iStatus=parseInt(extra.status,10);if(iStatus==201){_self.readdir(sPath.substr(0,sPath.lastIndexOf("/")),callback);
...
oBinary=null;}this.method="PUT";var _self=this;this.doRequest(function(data,state,extra){var iStatus=parseInt(extra.status,10);
...
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false&&!callback){throw oError;
}return callback&&callback.call(this,data,apf.ERROR,extra);}else{_self.getProperties(sPath,0,callback);
...
var iStatus=parseInt(extra.status,10);if(iStatus==400||iStatus==403||iStatus==409||iStatus==412||iStatus==423||iStatus==424||iStatus==501||iStatus==502||iStatus==500){var oError=WebDAVError.call(this,"Unable to move file '"+sFrom+"' to '"+sTo+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false&&!callback){throw oError;
...
var iStatus=parseInt(extra.status,10);if(iStatus==400||iStatus==423||iStatus==424){var oError=WebDAVError.call(this,"Unable to remove file '"+sPath+"'. Server says: "+apf.webdav.STATUS_CODES[String(iStatus)]);
if(this.dispatchEvent("error",{error:oError,bubbles:true})===false&&!callback){throw oError;
...
if(oProperties){for(var prop in oProperties){aCont.push("<D:"+prop,(oProperties[prop]?">"+apf.escapeXML(oProperties[prop])+"</D:"+prop+">":"/>"));
}}aCont.push("</D:",reportName,">");this.method="REPORT";this.doRequest(function(data,state,extra){var iStatus=parseInt(extra.status,10);
...
}function registerLock(data,state,extra){var iStatus=parseInt(extra.status,10),sPath=extra.url.replace(this.$server,""),oLock=this.$locks[sPath]||newLock.call(this,sPath);
...
}aOut.push("</D:propertyupdate>");return aOut.join("");}function parsePropertyPackets(oXml,state,extra,callback){var status=parseInt(extra.status,10);
if(status==403||status==401||!oXml){return callback?callback.call(this,null,state,extra):notAuth.call(this);
}if(typeof oXml=="string"){oXml=apf.getXml(oXml);}var aResp=$xmlns(oXml,"response",apf.webdav.NS.D);
var aOut=[];if(aResp.length){this.$regVar("authenticated",true);}var sPath;for(var sa=[],data,i=0,j=aResp.length;
...
var lastPos=-1;var masking=false;var oInput=this.$input;var pos=[];var initial,myvalue,format,fcase,replaceChar;
...
}}}this.$insertData(data||value);setPosition(myvalue.length);};var numpadKeys={"96":"0","97":"1","98":"2","99":"3","100":"4","101":"5","102":"6","103":"7","104":"8","105":"9","106":"*","107":"+","109":"-","110":".","110":"/"};
...
masking=true;this.$input[apf.isIphone?"onclick":"onmouseup"]=function(e){var pos=Math.min(calcPosFromCursor(),myvalue.length);
setPosition(pos);return false;};this.$input.onpaste=function(e){e=e||window.event;
e.returnValue=false;this.host.setValue(window.clipboardData.getData("Text")||"");
$setTimeout(function(){setPosition(lastPos);},1);};this.getValue=function(){if(this.includeNonTypedChars){return initial==this.$input.value?"":this.$input.value.replace(new RegExp(replaceChar,"g"),"");
}else{return myvalue.join("");}};this.setValue=function(value){this.$propHandlers.value.call(this,value);
};};this.setMask=function(m){if(!masking){this.$initMasking();}m=m.split(";");replaceChar=m.pop();
this.includeNonTypedChars=parseInt(m.pop(),10)!==0;var mask=m.join("");var validation="";
var visual="";var mode_case="-";var strmode=false;var startRight=false;var chr;
...
}}this.$input.value=visual;initial=visual;myvalue=[];replaceChar=replaceChar;};
function checkChar(chr,p){var f=format.substr(p,1);var c=fcase.substr(p,1);if(chr.match(new RegExp(_REF[f]))==null){return _FALSE_;
}if(c==">"){return chr.toUpperCase();}if(c=="<"){return chr.toLowerCase();}return chr;
}function setPosition(p){if(p<0){p=0;}if(apf.hasMsRangeObject){var range=oInput.createTextRange();
...
range.moveEnd("character",1);range.select();}else{if(typeof pos[p]=="undefined"){oInput.selectionStart=oInput.selectionEnd=pos[pos.length-1]+1;
lastPos=pos.length;return false;}oInput.selectionStart=pos[p];oInput.selectionEnd=pos[p]+1;
}lastPos=p;}function setCharacter(chr){if(pos.length&&pos[lastPos]==null){return false;
}chr=checkChar(chr,lastPos);if(chr==_FALSE_){return false;}if(apf.hasMsRangeObject){var range=oInput.createTextRange();
...
}}else{var val=oInput.value;var start=oInput.selectionStart;var end=oInput.selectionEnd;
oInput.value=val.substr(0,start)+chr+val.substr(end);oInput.selectionStart=start;
oInput.selectionEnd=end;}myvalue[lastPos]=chr;return true;}function deletePosition(p){if(pos[p]==null){return false;
}if(apf.hasMsRangeObject){var range=oInput.createTextRange();range.expand("textedit");
range.collapse();range.moveStart("character",pos[p]);range.moveEnd("character",1);
range.text=replaceChar;range.select();}else{var val=oInput.value;var start=pos[p];
var end=pos[p]+1;oInput.value=val.substr(0,start)+replaceChar+val.substr(end);oInput.selectionStart=start;
oInput.selectionEnd=end;}myvalue[p]=" ";}this.$insertData=function(str){if(str==this.getValue()){return;
}var i,j;try{if(!apf.hasMsRangeObject&&oInput.selectionStart==oInput.selectionEnd){setPosition(0);
...
}val=oInput.value;oInput.value=val.substr(0,pos[i])+chr+val.substr(pos[i]+1);}if(str.length){lastPos++;
...
}}if(str.length){lastPos++;}};function calcPosFromCursor(){var range,lt=0;if(!apf.hasMsRangeObject){lt=oInput.selectionStart;
...
}if(this.$data){var nodes=this.$data.childNodes;for(var i=nodes.length-1;i>=0;i--){nodes[i].destroy(true);
}}if(typeof data=="string"&&data.indexOf("<a:")>-1){this.$ext.innerHTML="";var doc=this.ownerDocument.$domParser.parseFromString("<a:application xmlns:a='"+apf.ns.apf+"'>"+data+"</a:application>","text/xml",{htmlNode:this.$ext,host:this});
