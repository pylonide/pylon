...
}}pathList.sort();var root=this.xmlRoot,_self=this;check("/",function(item){var paths=item.split("/");
...
if(!lastNode){next2(true);}else{next2();}});}},100);},function(err){next();});});
...
apf.aml.setElement("contents",apf.BindingRule);apf.defaults=function(struct,tagName){this.$init(tagName||"services",apf.NODE_HIDDEN,struct);
...
if(data.data){sa.push({toString:function(){return this.v;},data:data.data,v:(data.data.type=="file"?1:0)+""+data.data.name});
...
});if(window.require&&typeof require.def=="function"){var deps=[];if(deps.length){require([deps],function(){apf.start();
