...
(function(){
    ...
    this.removeFrom = function(defId, uniqueId) {
        var item = lookup[uniqueId].data;

        var i, def;
        if (item.buttons) {
            for (i = item.buttons.length - 1; i >= 0; --i) {
                def = item.buttons[i];
                if (def.uniqueId === defId) {
                    if (def.cache)
                        def.cache.removeNode();
                    item.buttons.splice(i, 1);
                }
            }
        }
        else if (item.sections) {
            for (i = item.sections.length - 1; i >= 0; --i) {
                def = item.sections[i];
                if (def.uniqueId === defId) {
                    if (def.cache)
                        def.cache.removeNode();
