...
jpf.XSDImplementation = function(){
    var typeHandlers = {
        ...
        "xsd:anyURI": function(value){
            return /^(?:\w+:\/\/)?(?:(?:[\w\-]+\.)+(?:[a-z]+)|(?:(?:1?\d?\d?|2[0-4]9|25[0-5])\.){3}(?:1?\d\d|2[0-4]9|25[0-5]))(?:\:\d+)?(?:\/([^\s\\\%]+|%[\da-f]{2})*)?$/i.test(value)
        },
        ...
        "xsd:long": function(value){
            return parseInt(value) == value && value >= -2147483648 && value <= 2147483647
        },
        ...
        "jpf:wechars": function(value){
            return /^[0-9A-Za-z\xC0-\xCF\xD1-\xD6\xD8-\xDD\xDF-\xF6\xF8-\xFF -\.',]+$/.test(value)
        },
    ...
    var simpleTypeHandler = {
        ...
        "pattern": function(xmlNode, func){
            func.push("if(!/^" + xmlNode.getAttribute("value").replace(/(\/|\^|\$)/g, "\\$1")
