    ...
    switch(type){
        case "add":
            if (undo) {
                ...
                var jmlNode = apf.document.createElement(this.addType);
                var htmlNode = options.htmlNode;
                var parentNode = (this.resize.getSelected() || apf.document.documentElement);
                ...
                var pos = apf.getAbsolutePosition(parentNode.oInt);
                ...
                var minwidth  = jmlNode.minwidth || parseInt(jmlNode.$getOption("main", "minwidth")) || 0;
                var minheight = jmlNode.minheight || parseInt(jmlNode.$getOption("main", "minheight")) || 0;
                var maxwidth  = jmlNode.maxwidth || parseInt(jmlNode.$getOption("main", "maxwidth")) || 10000;
                var maxheight = jmlNode.maxheight || parseInt(jmlNode.$getOption("main", "maxheight")) || 10000;
                
                jmlNode.setAttribute("width", Math.min(maxwidth, Math.max(minwidth, htmlNode.offsetWidth)));
                jmlNode.setAttribute("height", Math.min(maxheight, Math.max(minheight, htmlNode.offsetHeight)));
        ...
        case "remove":
            if (undo) {
                options.removedNode[1].insertBefore(options.removedNode[0], options.removedNode[2]);
                ...
                sel.removeNode();
                this.resize.grab(apf.window.focussed && apf.window.focussed.editable 
                    ? apf.window.focussed
        ...
        case "commit":
            if (undo) {
                ...
                if (this.started) {
                    this.started = false;
                    var items = apf.ContentEditable2.actions.transaction;
...
apf.Resize = function(){
    ...
    this.grab = function(oEl, options) {
        ...
        if (anchors) {
            apf.setStyleClass(nodes.n, anchors[0] ? "idegrabber_selected" : "", ["idegrabber_selected"]);
            apf.setStyleClass(nodes.e, anchors[1] ? "idegrabber_selected" : "", ["idegrabber_selected"]);
            apf.setStyleClass(nodes.s, anchors[2] ? "idegrabber_selected" : "", ["idegrabber_selected"]);
            apf.setStyleClass(nodes.w, anchors[3] ? "idegrabber_selected" : "", ["idegrabber_selected"]);
        }
        ...
        if (e.ctrlKey && this.type.length == 1) {
            ...
            if (selected[prop]) {
                if (prop == "right" && !this.left)
                    _self.setProperty("left", selected.oExt.offsetLeft);
                else if (prop == "bottom" && !this.top)
                    _self.setProperty("top", selected.oExt.offsetTop);
                ...
                switch(this.type) {
                    case "e":
                        _self.setProperty("right", pHtmlNode.offsetWidth - selected.oExt.offsetLeft - selected.oExt.offsetWidth);
                        break;
                    case "w":
                        _self.setProperty("left", selected.oExt.offsetLeft);
                        break;
                    case "n":
                        ...
                        break;
                    case "s":
                        _self.setProperty("bottom", pHtmlNode.offsetHeight - selected.oExt.offsetTop - selected.oExt.offsetHeight);
                        break;
                }
            }
            ...
            _self.minwidth = selected.minwidth || parseInt(selected.$getOption("main", "minwidth")) || 0;
            _self.minheight = selected.minheight || parseInt(selected.$getOption("main", "minheight")) || 0;
            _self.maxwidth = selected.maxwidth || parseInt(selected.$getOption("main", "maxwidth")) || 10000;
            _self.maxheight = selected.maxheight || parseInt(selected.$getOption("main", "maxheight")) || 10000;
...
apf.ElementDrawer = function (){
	...
	var p2 = document.body.appendChild(document.createElement("div"));
	...
	this.activate = function(){
		document.onmousemove = function(e){
			...
			if (q.style.display == "block"){
				...
				var ht = e.clientY - startY - 1;
				var min = Math.min(wt, ht);
				if (e.shiftKey) 
				    wt = ht = min;
				...
				q.style.right = wt < 0 ? (document.documentElement.offsetWidth - startX - 4) + "px" : "";
				
				q.style.bottom = ht < 0 ? (document.documentElement.offsetHeight - startY - 4) + "px" : "";
		...
		document.onmouseup = function(){
			if(q.offsetWidth > 10 && q.offsetHeight > 10) {
				var o = apf.ContentEditable2.execCommand("add", {htmlNode: q});
				_self.deactivate();
			}
