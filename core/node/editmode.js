...
jpf.KeywordServer = {
    ...
    /* #ifdef __WITH_EDITMODE
     ...
     //var re = new RegExp("^" + this.prefix.replace(/\./g, "\\.")), str = ["<lang id='EN'><group id='main'>"];
     var lut = {};
     for(key in this.words){
     nKey = key.split(".");//replace(/^.*\.(.*)$/, "$1");
     if(!lut[nKey[0]]) lut[nKey[0]] = [];
     //if(!lut[nKey[0]][nKey[1]]) lut[nKey[0]][nKey[1]] = []
     ...
     for(var i=0;i<lut[group].length;i++) str.push(lut[group][i]);
     str.push("</group>");
     }
     str.push("</lang>");
     //}
     ...
     if(this.automatch && !key && this.texts[str]) key = this.texts[str];
     if(!key) key = this.getNewKey();
     this.words[key] = str;
     this.texts[str] = key;
     if(!this.elements[key]) this.elements[key] = [];
     if(oEl) this.elements[key].push(oEl);
...
/* #ifdef __WITH_EDITMODE
 ...
 if(key == 27) EditServer.stopEdit(this.parentNode.editId, true);
 else if(key == 13) EditServer.stopEdit(this.parentNode.editId);
 }
 
 this.edit.onblur = function(){EditServer.stopEdit(this.parentNode.editId)}
 },
 ...
 var xmlNode = data.config ? XMLDatabase.selectSingleNode(data.config[data.counter][1], data.jmlNode) : XMLDatabase.getTextNode(data.jmlNode);
 if(!xmlNode) xmlNode = XMLDatabase.createNodeFromXpath(data.jmlNode, data.config[data.counter][1]);
 ...
 if(EditServer.edit.innerHTML) EditServer.edit.parentNode.onmouseout({});
 ...
 if(EditServer.isEditing || EditServer.edit.parentNode != this || (e || event).toElement == EditServer.edit) return;
 ...
 if(this.onupdateword) this.onupdateword(word, data.key);
 }
 else{
 ...
 //get ElementNode
 subNode = subNode.nodeType == 1 ? subNode : (subNode.nodeType == 3 || subNode.nodeType == 4 ? subNode.parentNode : subNode.ownerElement || subNode.selectSinglesubNode(".."));
 ...
 //setTimeout('alert("Switch to");value1 = jpf.KeywordServer.getWordListXml();jpf.KeywordServer.setWordListXml(jpf.KeywordServer.getWordListXml(true), "sub");', 3000);
 //setTimeout('alert("Switch back");jpf.KeywordServer.setWordListXml(value1, "sub");', 6000);
 #endif */
// #ifdef __WITH_LANG_SUPPORT && !__WITH_EDITMODE
