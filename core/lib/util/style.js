...
//#ifdef __WITH_OFFSET
if ("getBoundingClientRect" in document.documentElement) {
    apf.offset = function(elem, options) {
        // setting offset?
        if (options)
            return apf.offset.setOffset(elem, options);

        if (!elem || !elem.ownerDocument)
            return null;

        if (elem === elem.ownerDocument.body)
            return apf.offset.bodyOffset(elem);

        apf.offset.initialize(true);
        ...
        var box        = elem.getBoundingClientRect(),
            ...
            top        = box.top  + (self.pageYOffset || apf.offset.boxModel && docElem.scrollTop  || body.scrollTop) - clientTop,
            left       = box.left + (self.pageXOffset || apf.offset.boxModel && docElem.scrollLeft || body.scrollLeft) - clientLeft;
    ...
    apf.offset = function(elem, options) {
        ...
        while ((elem = elem.parentNode) && elem !== body && elem !== docElem) {
            if (apf.offset.supportsFixedPosition && prevComputedStyle.position === "fixed")
                break;
            ...
            if (elem === offsetParent) {
                ...
                if (apf.offset.doesNotAddBorder && !(apf.offset.doesAddBorderForTableAndCells && /^t(able|d|h)$/i.test(elem.nodeName))) {
            ...
            if (apf.offset.subtractsBorderForOverflowNotVisible && computedStyle.overflow !== "visible") {
                top  += parseFloat(computedStyle.borderTopWidth) || 0;
                left += parseFloat(computedStyle.borderLeftWidth) || 0;
            }
...
apf.extend(apf.offset, {
    initialize: function(boxOnly) {
        if (boxOnly) {
            ...
            var innerDiv, checkDiv, td,
                body          = document.body,
                container     = document.createElement("div"),
                bodyMarginTop = parseFloat(apf.getStyle(body, "marginTop")) || 0,
    ...
    bodyOffset: function(body) {
        ...
        if (apf.offset.doesNotIncludeMarginInBodyOffset) {
            top  += parseFloat(apf.getStyle(body, "marginTop")) || 0;
            left += parseFloat(apf.getStyle(body, "marginLeft")) || 0;
        }
    ...
    setOffset: function(elem, options) {
        ...
        if (/static/.test(apf.getStyle(elem, "position")))
            elem.style.position = "relative";
        var curOffset = apf.offset(elem),
            curTop    = parseInt(apf.getStyle(elem, "top"),  10) || 0,
            curLeft   = parseInt(apf.getStyle(elem, "left"), 10) || 0;

        if (typeof options == "function")
            options = options.call(elem, curOffset);
