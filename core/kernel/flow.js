...
jpf.flow.Block = function(htmlElement, canvas, other){
	...
	this.hasConnectionWithInput = function(input){
		for (var id in this.canvas.htmlConnectors) {
			if(this.canvas.htmlConnectors[id].source.id == this.id && this.canvas.htmlConnectors[id].output == input){
				return {
					connector : this.canvas.htmlConnectors[id],
					type : "S"
				};
			} 
			if(this.canvas.htmlConnectors[id].destination.id == this.id && this.canvas.htmlConnectors[id].input == input){
				return {
					connector : this.canvas.htmlConnectors[id],
					type : "D"
				};
	...
	this.getConnection = function(htmlDestinationNode, output, input){		
		for(var id in this.canvas.htmlConnectors){				
			if(this.canvas.htmlConnectors[id].source.id == this.id && this.canvas.htmlConnectors[id].destination.id == htmlDestinationNode.getAttribute("id")){
				if(this.canvas.htmlConnectors[id].output == output && this.canvas.htmlConnectors[id].input == input){
		...
		this.oExt.onmousedown = function(e){		
			...
			if(hasConnection){
				if (_self.objBlock.canvas.mode == "connection-change") {
					var t_source = hasConnection.type == "S" ? hasConnection.connector.source : hasConnection.connector.destination;
					var t_destination = hasConnection.type == "S" ? hasConnection.connector.destination : hasConnection.connector.source;
					var t_output = hasConnection.type == "S" ? hasConnection.connector.output : hasConnection.connector.input;
					var t_input = hasConnection.type == "S" ? hasConnection.connector.input : hasConnection.connector.output;				
					...
					new jpf.flow.addConnector(_self.objBlock.canvas, t_destination, vMB, {output: t_input, input: null, xmlNode:null});
				...
				if(_self.objBlock.canvas.mode == "connection-add"){
					new jpf.flow.addConnector(_self.objBlock.canvas, _self.objBlock, vMB, {output : _self.input_number, input : null, xmlNode : null});	
...
jpf.flow.Segment = function(id, objConnector){
			...
			for(var id in objConnector.canvas.htmlConnectors){
				if(objConnector.canvas.htmlConnectors[id] == objConnector){
					for(var id2 in objConnector.canvas.htmlConnectors[id].htmlConnectorSegments){
						objConnector.canvas.htmlConnectors[id].htmlConnectorSegments[id2].htmlElement.style.border = "1px solid black";
					}					
				}
			}		
		}					
		...
		for(var id in jpf.flow.htmlCanvases){
			for(var id2 in jpf.flow.htmlCanvases[id].htmlConnectors){
				if(jpf.flow.htmlCanvases[id].htmlConnectors[id2].htmlConnectorSegments[this.htmlElement.id]){
					jpf.flow.htmlCanvases[id].htmlConnectors[id2].htmlConnectorSegments[this.htmlElement.id].htmlElement.parentNode.removeChild(jpf.flow.htmlCanvases[id].htmlConnectors[id2].htmlConnectorSegments[this.htmlElement.id].htmlElement);	
					delete jpf.flow.htmlCanvases[id].htmlConnectors[id2].htmlConnectorSegments[this.htmlElement.id];
				}
			}
		}					
	}
	...
	this.getEndX = function(){		
		switch(this.orientation){
			case jpf.flow.LEFT: return this.startX - this.length;
			case jpf.flow.RIGHT: return this.startX + this.length;
			case jpf.flow.DOWN: return this.startX;
			case jpf.flow.UP: return this.startX;
		}
	}
	...
	this.getEndY = function(){		
		switch(this.orientation){
			case jpf.flow.LEFT: return this.startY;
			case jpf.flow.RIGHT: return this.startY;
			case jpf.flow.DOWN: return this.startY + this.length;
			case jpf.flow.UP: return this.startY - this.length;
		}
	}
...
jpf.flow.ConnectorManager = function(objBlock, input_number){	
	if(jpf.flow.connectorTemp.length == 0){
		jpf.flow.connectorTemp.push({objBlock : objBlock, input_number : input_number });
	}
	else{
...
jpf.flow.getPosition = function(i, objBlock, newposition, w, h){	
	...
	var input = {position:null, x:null, y:null};
	...
	//Horizontal
	if (fliph == 1 && flipv == 0) {		
		if ((input.position == "left" || input.position == "right") && (rotation == 0 || rotation == 180)) {
			input.position = jpf.flow.invertPosition(input.position);
		}
		else if ((input.position == "top" || input.position == "bottom") && (rotation == 0 || rotation == 180)) {
			input.x = w - input.x;	
			if(jpf.isIE || jpf.isOpera){
				input.x--;
			}		
		}			
		else if ((input.position == "top" || input.position == "bottom") && (rotation == 90 || rotation == 270)) {
			if(jpf.isIE || jpf.isOpera){
				input.position == "bottom" ? input.x -=1 : input.x++;				
			}					
		}
		else if ((input.position == "left" || input.position == "right") && (rotation == 90 || rotation == 270)) {
			input.position = jpf.flow.invertPosition(input.position);										
		}
	}			
	//Vertical
	else if(fliph == 0 && flipv == 1){		
		if((input.position == "top" || input.position == "bottom") && (rotation == 0 || rotation == 180)){
			input.position = jpf.flow.invertPosition(input.position);
			input.y = h-input.y+2;						
		}	
		else if((input.position == "top" || input.position == "bottom") && (rotation == 90 || rotation == 270)){				
		...
		else if((input.position == "left" || input.position == "right") && (rotation == 0 || rotation == 180)){
			input.y = h-input.y;
			if(jpf.isIE || jpf.isOpera){
				input.y--;
			}							
...
jpf.flow.getOrientation = function(position){
	switch(position){
		case 'top': return jpf.flow.UP; break;
		case 'bottom': return jpf.flow.DOWN; break;
		case 'left': return jpf.flow.LEFT; break;
		case 'right': return jpf.flow.RIGHT; break;
		case 'auto': return jpf.flow.AUTO; break;
	}
}
...
jpf.flow.drawStrategy = function(connector){
	...
	this.paint = function(){			
		...
		this.createStartLine(this.startSegment, jpf.flow.STICKS_LENGHT, sourceTop, sourceLeft, sourceWidth, sourceHeight, destinationTop, destinationLeft, destinationWidth, destinationHeight, this.realOutput);	
		this.createStartLine(this.endSegment, jpf.flow.STICKS_LENGHT, destinationTop, destinationLeft, destinationWidth, destinationHeight, sourceTop, sourceLeft, sourceWidth, sourceHeight, this.realInput);		
	...
	this.createRestLines = function (objSSegment, objESegment){		
		...
		if (l > dl) {
			//dX is on the left
			if(t > dt){position = "TL";}//dY is above
			else if(t < dt){position = "BL";}//dY is under
			else{position = "ML";}//dY is equal
		}
		else if (l < dl) {
			//dX jest po prawej
			if(t > dt){position = "TR";}//dY jest nad
			else if(t < dt){position = "BR";}//dY jest pod
			else{position = "MR";}//dY jest na równi
		}
		else{
			//X jest na równi
			if(t > dt){position = "TM";}//dY jest nad
			else if(t < dt){position = "MM";}//dY jest pod
			else{position = "BM";}//dY jest na równi
		}			
...
jpf.flow.ConnectorEnd = function(htmlElement, connector, side){
	...
	this.destroy = function(){			
		if(this.connector.htmlConnectorEnds[this.htmlElement.id]){
			this.connector.htmlConnectorEnds[this.htmlElement.id].htmlElement.parentNode.removeChild(this.connector.htmlConnectorEnds[this.htmlElement.id].htmlElement);
	...
	this.repaint = function(){
		...
		if(this.htmlElement.tagName.toLowerCase() == "img" && this.orientation != orientation){
			...
			switch(orientation){
				case jpf.flow.UP: orientationSuffix = "u"; break;
				case jpf.flow.DOWN: orientationSuffix = "d"; break;
				case jpf.flow.LEFT: orientationSuffix = "l"; break;
				case jpf.flow.RIGHT: orientationSuffix = "r"; break;
			}
...
jpf.flow.SideConnectorLabel = function(connector, htmlElement, side){
	...
	this.destroy = function(){					
		if(this.connector.htmlLabels[this.htmlElement.id]){
			this.connector.htmlLabels[this.htmlElement.id].htmlElement.parentNode.removeChild(this.connector.htmlLabels[this.htmlElement.id].htmlElement);	
	...
	this.repaint = function(){
		...
		if(this.side == jpf.flow.START){	
			...
			if((segment.orientation & jpf.flow.HORIZONTAL) != 0 && top < this.connector.getEndSegment().getEndY())
				top -= this.htmlElement.offsetHeight;
			if((segment.orientation & jpf.flow.VERTICAL) != 0 && left < this.connector.getEndSegment().getEndX())
				left -= this.htmlElement.offsetWidth;
		}
		else{	
			...
			if(segment.orientation == jpf.flow.DOWN)
				top -= this.htmlElement.offsetHeight;
			if((segment.orientation & jpf.flow.HORIZONTAL) != 0 && top < this.connector.getStartSegment().startY)
				top -= this.htmlElement.offsetHeight;
			if((segment.orientation & jpf.flow.VERTICAL) != 0 && left < this.connector.getStartSegment().startX)
				left -= this.htmlElement.offsetWidth;
		}
...
jpf.flow.addConnector = function(objCanvas, objBlockSource, objBlockDestination, other){
	...
	if(!jpf.flow.isConnector(htmlElement)){		
		this.newConnector = new jpf.flow.Connector(htmlElement, objCanvas, objBlockSource, objBlockDestination, other);
