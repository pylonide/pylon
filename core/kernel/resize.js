		...
		if(e.shiftKey){			
			...
			if(posY == "bottom" || posY == "top"){
				if(posX == "left" || posX == "right"){
					htmlBElement.style.width = (posX == "right" ? currentMouseX - l - cx - m - pl - fix2l : (posX == "left" ? w + l - currentMouseX + (ows - cx) + m - pl - fixl : w))+"px";
					htmlBElement.style.height = parseInt(htmlBElement.style.width)*proportion+"px";					
				}	
			}			
		}
		else{
			htmlBElement.style.width = Math.max(objSquare.resize.scales.dwidth, (posX == "right" ? currentMouseX - l - cx - m - pl - fix2l : (posX == "left" ? w + l - currentMouseX + (ows - cx) + m - pl - fixl : w)))+"px";
			htmlBElement.style.height = Math.max(objSquare.resize.scales.dheight, (posY == "bottom" ? currentMouseY - t - cy - m - pt - fix2t : (posY == "top" ? h + t - currentMouseY + (ohs - cy) + m + pt - fixt : h)))+"px";
		}			
				
		htmlBElement.style.left = (posX == "left" ? currentMouseX - (ows - cx) - m + pl + fixl : l)+"px";
		htmlBElement.style.top = (posY == "top" ? currentMouseY - (ohs - cy) - m - pt + fixt : t)+"px";
        ...
        this.repaint = function(){
            if(this.visible){
				...
				if(jpf.isGecko || jpf.isOpera){					
					oW -= borderl; oH -= bordert;
				}
				
				t = (posY == "top" ? t-oH-this.resize.m :(posY == "middle" ? (2*t+h-oH + bordert)/2 : t+h+this.resize.m + bordert));   
                l = (posX == "left" ? l-oW-this.resize.m : (posX == "middle" ? (2*l+w-oW + borderl)/2 : l+w+this.resize.m + borderl));								
                c = (posY == "middle" ? "w-resize" : (posX == "middle" ? "n-resize" : (posY+posX == "topleft" || posY+posX == "bottomright") ? "nw-resize" : "ne-resize"));
...
jpf.resize = function(){    
    ...
    this.m  = 6; //block margin
   	this.scales = {scalex : false, scaley : false, scaleratio : false};
