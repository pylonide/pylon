...
jpf.chart.axis.draw = {
    ...
    grid3D : function(l,e){
        ...
        e.allocDone(l);
        var sx = l.style.xsteps, sy = l.style.ysteps;
        var c = e.optimize([
            e.begin3D(e,l),
...
jpf.chart.graph = jpf.subnode(jpf.NODE_HIDDEN, function(){
    ...
    this.$loadJml = function(x,obj, order, root){
        ...
        this.v_info  = [];
        // x / y value array
        var p,v,k,l,t = (new Date()).getTime()*0.001;
        if(this.series){
            ...
            for( v = 0; v < p.length; v++ ){
                ...
                if( l = k.length > 0 ){
					if(l==1)this.v_yval[v] = parseFloat(k[0]);
						...
						this.v_xval[v] = parseFloat(k[0]);
                        this.v_yval[v] = parseFloat(k[1]);
...
jpf.chart.graph.draw = {
   ...
   _line2D: {
    ...
    line2D : function( l, e, d ){
        ...
        var s = l.style, wrap = s.graph.weight*8;
    ...
    line3D : function( l, d ){
        var e = jpf.draw, s = l.style, wrap = s.graph.weight*4; 
...
"if(!l.v_sum){for(i=_yv.length-1,j=0,_mi=10000000,_ma=-10000000;i>=0;--i)j+=((m=_yv[i])<_mi?_mi=m:m)>_ma?_ma=m:m;l.v_avg=(l.v_sum=j)/_yv.length;l.v_min=_mi;l.v_max=_ma;}",
    ...
    seriesX2 : function(l) {
        ...
        return {
            ...
            forx : " && _lx<=vx2",
            incx : "",
            ifx  : "if( (!_sf && (x==_len-1 || _xv[x+1]>=vx1) && ++_sf && (xfirst=l.xfirst=x || 1)) || _sf)",
            x      : "(_lx=_xv[x])",
            y      : "_yv[x]"
        };
    },
...
jpf.chart.layer = jpf.subnode(jpf.NODE_HIDDEN, function(){
    ...
    this.$loadJml = function(x,obj){
        ...
        this.$draw      = jpf.chart.layer.draw[this.type](this, this.datasource);
    }
});
