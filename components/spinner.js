jpf.spinner = function(pHtmlNode, tagName){
	jpf.register(this, tagName || "spinner", GUI_NODE);/** @inherits jpf.Class */
	...
	*********************************************************************/
	...
	this.__clear = function(){
		...
		if(this.oInput.tagName.toLowerCase().match(/input|textarea/i)) this.oInput.value = "";
		else{
	...
	this.__enable = function(){this.oInput.disabled = false;}
	this.__disable = function(){this.oInput.disabled = true;}
	this.select = function(){this.oInput.select();}
	this.deselect = function(){this.oInput.deselect();}
	...
	************************/
	this.keyHandler = function(key){		
		if(key < 8 || (key > 8 && key < 37 ) || (key > 40 && key < 46) || (key > 46 && key < 48) || (key > 57 && key < 109) || key > 109) return false;
	...
	**********/
	...
	this.draw = function(){		
		...
		this.oInput.style.width =  this.oInput.parentNode.offsetWidth - 6 - Math.max(this.oButtonPlus.offsetWidth, this.oButtonMinus.offsetWidth) + "px";
		...
		if(!jpf.hasContentEditable && !this.oInput.tagName.toLowerCase().match(/input|textarea/)){
			...
			if(this.oExt == node) this.oExt = this.oInput;
		}
		
		this.oInput.onselectstart = function(e){if(!e) e = event;e.cancelBubble = true}
