jpf.calendar = function(pHtmlNode, tagName){
    ...
    **********/
    ...
    this.nextYear = function(){this.redraw(currentMonth, currentYear + 1);}
    this.prevYear = function(){this.redraw(currentMonth, currentYear - 1);}
			...
			for (var i = 0; i < 6; i++) {
	            ...
	            for(var j=0;j < 8;j++){
	                ...
	                var oCell = this.__getLayoutNode("Cell");
	               	if(j>0){
						oCell.setAttribute("onmouseover", 'if(this.className.indexOf("disabled") > -1) return; jpf.lookup(' + this.uniqueId + ').__setStyleClass(this, "hover");');
				    	oCell.setAttribute("onmouseout", 'var o = jpf.lookup(' + this.uniqueId + ').__setStyleClass(this, "", ["hover"]);');	
						oCell.setAttribute("onmousedown", 'var o = jpf.lookup(' + this.uniqueId + ');if(this.className.indexOf("prev") > -1){o.clickDay(this.innerHTML, "prev");}else if(this.className.indexOf("next") > -1){o.clickDay(this.innerHTML, "next");}else{o.clickDay(this.innerHTML);}');	
					}					
