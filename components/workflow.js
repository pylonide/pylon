...
jpf.workflow = function(pHtmlNode) {
    jpf.register(this, "workflow", jpf.NODE_VISIBLE);/** @inherits jpf.Class */
    ...
    this.$updateModifier = function(xmlNode, htmlNode) {
        ...
        var curzindex = parseInt(jpf.getStyle(htmlNode, jpf.descPropJs
            ? "zIndex"
            : "z-index")) || 1;
        if (curzindex != zindex) {
        ...
        for (var i = 0; i < cNew.length; i++) {
            ...
            if (!found) {
                ...
                new jpf.flow.addConnector(
                    _self.objCanvas,
                    blockId[this.applyRuleSetOnNode("id", xmlNode)],
                    blockId[ref],
                    {
                        output : output,
                        input  : input,
    ...
    this.$add = function(xmlNode, Lid, xmlParentNode, htmlParentNode, beforeNode) {
        ...
        if (this.hasFeature(__RENAME__)) {
            ...
            elSelect.setAttribute(
                this.itemSelectEvent || "onmousedown", 'var o = jpf.lookup(' + 
                this.uniqueId + ');if(!o.renaming && o.hasFocus() && jpf.xmldb.isChildOf(o.selected, this, true) && o.value)'+
            ...
            elSelect.setAttribute(
                this.itemSelectEvent || "onmousedown", 'var o = jpf.lookup(' + this.uniqueId +
    ...
    this.$fill = function() {
        ...
        for (var id in xmlBlocks) {
            ...
            if (!jpf.flow.isBlock(htmlElement)) {
                ...
                if (xmlBlocks[id].getAttribute("type")) {
                    if (this.template) {
                        var elTemplate = this.template.selectSingleNode("//element[@type='" +
                            this.applyRuleSetOnNode("template", xmlBlocks[id]) + "']");
                ...
                var other = {
                    lock      : this.applyRuleSetOnNode("lock", xmlBlocks[id])
                        ? this.applyRuleSetOnNode("lock", xmlBlocks[id])
                        : 0,
                    flipv     : this.applyRuleSetOnNode("flipv", xmlBlocks[id])
                        ? this.applyRuleSetOnNode("flipv", xmlBlocks[id])
                        : 0,
                    fliph     : this.applyRuleSetOnNode("fliph", xmlBlocks[id])
                        ? this.applyRuleSetOnNode("fliph", xmlBlocks[id])
                        : 0,
                    rotation  : this.applyRuleSetOnNode("rotation", xmlBlocks[id])
                        ? this.applyRuleSetOnNode("rotation", xmlBlocks[id])
                        : 0,
                    inputList : inputList,
