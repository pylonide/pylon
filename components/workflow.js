  ...
  //if(resize) resize.show();
    ...
    ************************/
    ...
    this.keyHandler = function(key, ctrlKey, shiftKey, altKey){
        ...
        switch(key){
            ...
            case 46:
                //DELETE
                if (_self.objCanvas.disableremove) return;
    ...
    ************************/
    ...
    this.removeConnector = function(xmlNodeArray){    
        ...
        for(var i = 0; i<xmlNodeArray.length; i++){            
             changes.push({func : "removeNode", args : [xmlNodeArray[i]]});
        }
    ...
    **********/
    ...
    var cCurrent = connectors[this.applyRuleSetOnNode("id", xmlNode)];
        ...
        if(cCurrent){
            for(var i=0; i<cCurrent.length; i++){
                ...
                for(j=0; j<cNew.length; j++){                
                    if(
                            (cCurrent[i].ref == this.applyRuleSetOnNode("ref", cNew[j])) &&
        ...
        for(var i=0; i<cNew.length;i++){ 
            ...
            for(j=0; j<cCurrent.length; j++){                
                if(
                        (cCurrent[j].ref == this.applyRuleSetOnNode("ref", cNew[i])) && 
            ...
            if(!found){                    
                ...
                new jpf.flow.addConnector(_self.objCanvas, blockId[this.applyRuleSetOnNode("id", xmlNode)], blockId[ref], {output : output, input : input, xmlNode : cNew[i]});
            }
        }
    ...
    this.__fill = function(){        
        ...
        for(var id in xmlBlocks){        
            ...
            var htmlElement = jpf.XMLDatabase.findHTMLNode(xmlBlocks[id], this);                    
        ...
        for(var id in xmlBlocks){        
            if(connectors[id]){                    
                ...
                for(var i=0; i<c.length; i++){                
                    ...
                    if(!Con){                        
                        new jpf.flow.addConnector(_self.objCanvas, blockId[id], blockId[c[i].ref], {output: c[i].output, input : c[i].input, xmlNode : c[i].xmlNode});                        
                    }                    
                }
            }                
        }            
    ...
    **********/
    ...
    this.__loadJML = function(x){
        ...
        jpf.getData(this.jml.getAttribute("loadtemplate"), 
                  null, 
                                    function(data, state, extra){
                    
                                        if(state != __HTTP_SUCCESS__){                                            
                                            addEventToConsole("An error has occurred: " + extra.message,2);
                                            return;
                                        }  
                                        //jpf.alert_r(data);                                        
    ...
    this.loadTemplate = function(data){    
        this.template = jpf.XMLDatabase.getBindXmlNode(data);                
