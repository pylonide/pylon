	...
	if(jpf.isGecko) {
        for(var i = 0; i <= jpf.chart.height; i++) {
            if(i == temp) {
				...
				var temp2 = y_max > 0 
								? y_max - y_max%jpf.chart.value_per_y_axis - counter * Math.abs(jpf.chart.value_per_y_axis)
								: y_max - y_max%jpf.chart.value_per_y_axis - (counter + 1) * Math.abs(jpf.chart.value_per_y_axis);
