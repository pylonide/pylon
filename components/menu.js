...
jpf.menu = function(pHtmlNode){
    ...
    *********************************************************************/
    ...
    this.display = function(x, y, noanim, opener, xmlNode){
        ...
        this.focus();
        if(this.lastFocus) this.lastFocus.__focus();
    ...
    this.getSelectedValue = function(group){
        ...
        for(var i=0;i<values.length;i++){
            ...
            if(htmlNode.className.indexOf("checked") > -1) return values[i];
        }
    ...
    this.setSelected = function(group, value){
        ...
        for(var i=0;i<values.length;i++){
            var htmlNode = document.getElementById(group + ":" + values[i] + ":" + this.uniqueId);
            if(values[i] == value) this.__setStyleClass(htmlNode, "selected");
            else this.__setStyleClass(htmlNode, "", ["selected"]);
        }
    }
    
    this.toggleChecked = function(value){
        var htmlNode = document.getElementById(value + ":" + this.uniqueId);
        if(htmlNode.className.match(/checked/)) this.__setStyleClass(htmlNode, "", ["checked"]);
        else this.__setStyleClass(htmlNode, "checked");
    }
    ...
    ************************/
    this.addItem = function(xmlNode){
        ...
        if(isRadio && selected) this.__setStyleClass(elItem, "selected");
        if(isCheckbox && checked) this.__setStyleClass(elItem, "checked");
        if(isRadio && group){
        ...
        if(isRadio) elItem.setAttribute("onmousedown", 'jpf.lookup(' + this.uniqueId + ').setSelected("' + group + '", "' + value + '");'); 
        if(isCheckbox) elItem.setAttribute("onmousedown", 'jpf.lookup(' + this.uniqueId + ').toggleChecked("' + value + '");'); 
    ...
    **********/
    ...
    this.draw = function(){
        ...
        for(var i=0;i<nodes.length;i++){
            ...
            if(tagName.match(/^(?:item|radio|check)$/)) this.addItem(nodes[i]);
