    ...
    this.__loadJML = function(x){
        ...
        this.addEventListener("onclick", function(e){
            ...
            if (this.submission) {
                ...
                if (!submission) 
                    throw new Error(0, jpf.formErrorString(0, this, "Submission", "Could not find submission to execute action on '" + this.submission + "'", this.jml));
                ...
                if (this.target) {
                    //#ifdef __DEBUG
                    if (!self[this.target]) 
                        throw new Error(0, jpf.formErrorString(0, this, "Clicking on Button", "Could not find target to execute action on '" + this.target + "' with action '" + this.action + "'", this.jml));
                    //#endif
                    ...
                    if (!target) {
                        ...
                        //#ifdef __DEBUG
                        if (!target) 
                            throw new Error(0, jpf.formErrorString(0, this, "Clicking on Button", "Could not find target to for action '" + this.action + "'", this.jml));
                        //#endif
                    }
                }
            ...
            if (!target[this.action])
                throw new Error(0, jpf.formErrorString(0, this, "Clicking on Button", "Could not find action on target.", this.jml));
            //#endif
