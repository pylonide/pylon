...
function RTEHelper(oRTEToBe, skin){
    ...
    this.fixContent = function(){
        ...
        for(var i=0;i<img.length;i++){
            if(!img[i].src.match(/^https?:\/\/|upload_needed/)){
                img[i].src = (self.this.mediaPath || top.this.mediaPath || "") + "upload_needed.gif";
                found = true;
            }
        }
        
        if(found) alert("One or more images have to be uploaded before they can be used. They have been replaced.");
    }
    
    this.insertImage = function(path){
        if(document.activeElement != this.oExt) this.oExt.focus();
    ...
    ****************/
    ...
    this.mHoverOver = function(dark){
        if(this.hideBorder) this.style.border = "1px dotted " + (dark ? "black" : "gray");
    ...
    this.deActivate = function(fromRoot){
        if(!fromRoot && self.RTETemplateViewer) RTETemplateViewer.active = null;
    ...
    ****************/
    ...
    this.Init = function(showBorder, argOverflow, font, wrap, align){
        ...
        with(this.oExt.style){
            ...
            if(wrap || wrap == null) wordWrap = "break-word";
        ...
        //Set contentEditable
        if(top.IS_IE) this.oExt.contentEditable = true;
        else document.designMode = "on";
        
        //Interaction
        this.oExt.host = this;
        this.oExt.onclick = function(){this.host.activate();}
        this.oExt.onkeypress = function(){return this.host.keyHandler(event.keyCode);}
        this.oExt.ondrop = this.oExt.onpaste = function(){__TO=this.host;setTimeout("__TO.fixContent();__TO=null;")}
        if(self.RTETemplateViewer) this.oExt.onkeydown = this.oExt.onkeyup = this.oExt.onmouseup = function(){RTETemplateViewer.redoButtons();}
