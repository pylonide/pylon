...
package
{
    ...
    public class File
    {
        ...
        private function updateProgress(bytes_loaded:uint, complete:Boolean = false):void
        {
            ...
            if (bytes_loaded && !complete) {
                ...
                if (length > 1) {
                    ...
                    if (length > parent.options.progressGraphSize)
                        length = parent.options.progressGraphSize;
            ...
            for (i = 0; i < parent.fileList.length; i++) {
        ...
        private function handleTimeout(event:TimerEvent):void
        {
            ...
            complete('timeLimit (' + options.timeLimit + 's) exceeded', 'timeout', 0);
        }
        ...
        {
            complete(event.text, event.type, 0);
        }
        ...
        private function complete(text:String = null, error:String = null, code:int = 0):void
        {
        ...
        public function setOptions(options_init:Object = null):void
        {
            if (options_init != null) {
                for (var prop:String in options) {
                    if (options_init.hasOwnProperty(prop))
                        options[prop] = options_init[prop];
                }
            }
        }
        ...
        public function start():Boolean
        {
            ...
            var options_parent:Object = parent.options,
                merged:Object = new Object(),
                prop:String;
            
            for (prop in options) {
                merged[prop] = (options[prop] != null) 
                    ? options[prop] 
                    : options_parent[prop];
            }
                ...
                if (merged.data != null && merged.data != false) {
                    if (merged.mergeData && options.data && options_parent.data) {
                        if (options.data is String && options_parent.data is String) {
                            ...
                            for (prop in options_parent.data) {
                    ...
                    if (merged.data is String)
                        data.decode(merged.data);
                    else
                        for (var key:Object in merged.data)
                            data[key] = merged.data[key];
                    url_request.data = data;
                }
            
                url_request.method = URLRequestMethod[(merged.method) 
        ...
        public function remove():Boolean
        {
            ...
            var idx:int = parent.fileList.indexOf(this);
        ...
        public function validate():Boolean
        {
            ...
            if (parent.options.fileSizeMin > 0 
              && reference.size < parent.options.fileSizeMin) {
                validationError = 'sizeLimitMin';
                return false;
            }
            
            if (parent.options.fileSizeMax > 0 
              && reference.size > parent.options.fileSizeMax) {
        ...
        static public function exportMany(files:Array):Array
        {
            ...
            return files.map(function(current:File, i:uint, self:Array):Object {
